...
class LeadController extends FormController
{
    ...
    public function getDatabase() {
        ...
        $username = "shoptech_main";
        $password = "pass@1234";
        $dbname = "shoptech_demo";
        $this->conn = mysqli_init();
        $this->conn->real_connect('localhost', $username, $password, $dbname);
        // Create connection
    ...
    public function indexAction($page = 1)
    {
        ...
        $customers = $this->conn->query("SELECT * FROM stm_customer");
    ...
    public function viewAction($objectId)
    {
        ...
        $SelectCustomer = $this->conn->query("SELECT * FROM stm_customer WHERE id_customer = ".$objectId);
        $SelectedCustomer;
        ...
        $getcustomers = $this->conn->query("SELECT * FROM stm_customer WHERE id_customer = ". $objectId);
        $customer = [];
        ...
        $leadClass = new Lead();
        $getFields = [];
        foreach ($leadClass->getvariables() as $key => $value) {
            $getFields[$key] = $customer[0][$key];
            //print_r($key);
        }

        $getFields['id'] = $customer[0]['id_customer'];
