...
class AdminProductsControllerCore extends AdminController
{
    ...
    protected $enable_analytics = true;
    ...
    public function setMedia()
    {
        ...
        $this->addCss(__PS_BASE_URI__.'js/jquery/plugins/chosen/jquery.chosen.css');

        $this->addJs(__PS_BASE_URI__.'js/jquery/ui/jquery.ui.widget.min.js');
        $this->addJs(__PS_BASE_URI__.'js/jquery/plugins/jquery.chosen.js');
    ...
    public function renderList()
    {
        ...
        foreach ($this->_list as &$l) {
            ...
            $combination = new Combination(Db::getInstance()->getValue("SELECT id_product_attribute FROM " . _DB_PREFIX_ . "product_attribute WHERE default_on = 1 AND id_product = '{$l['id_product']}'"));

            $attributes = $combination->getAttributes($this->context->language->id);
            ...
            foreach ($attributes as $index => $attribute) {
                ...
                $combination_name .= $attribute['name'] . ' : ' . $attribute['value'];
            }

            $label = "{$l['name']}{$combination_name}";
            ...
            $query = [
                ...
                'actionToWidgetize' => 'getRowEvolutionPopover',
                'idSite' => $PIWIK_SITEID,
                'period' => $period,
                'token_auth' => Configuration::get('PIWIK_TOKEN_AUTH'),
                'language' => 'en',
                'date' => $date,
                'apiMethod' => 'Goals.getItemsName',
                // 'label' => '@' . $label,
            ...
            $l['analytics_query'] = http_build_query($query);
            $l['analytics_query'] .= '&label=%40' . $label;
    ...
    public function renderForm()
    {
        ...
        ], '@' . $this->object->name[$this->context->language->id]);
    ...
    protected function _displaySpecificPriceModificationForm($defaultCurrency, $shops, $currencies, $countries, $groups)
    {
        ...
        $content .= '<br/><div class="panel">
		<h3 class="card-header">'.$this->l('Priority management').'</h3>
        ...
        <div class="panel-body">
    		<div class="form-group">
    			...
    			<div class="input-group col-lg-9">
    				<select class="form-control" id="specificPricePriority1" name="specificPricePriority[]">
    				...
    				<select class="form-control" name="specificPricePriority[]">
    		...
    		<div class="panel-footer">
    				<a href="'.$this->context->link->getAdminLink('AdminProducts').($page > 1 ? '&submitFilter'.$this->table.'='.(int) $page : '').'" class="btn btn-default"><i class="process-icon-cancel"></i> '.$this->l('Cancel').'</a>
    				<button id="product_form_submit_btn"  type="submit" name="submitAddproduct" class="btn btn-primary pull-right" disabled="disabled"><i class="process-icon-loading"></i> '.$this->l('Save').'</button>
    				<button id="product_form_submit_btn"  type="submit" name="submitAddproductAndStay" class="btn btn-primary pull-right" disabled="disabled"><i class="process-icon-loading"></i> '.$this->l('Save and stay').'</button>
    			</div>
    		</div>
    	</div>';
