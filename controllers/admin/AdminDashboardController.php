...
class AdminDashboardControllerCore extends AdminController
{
    ...
    public function renderView()
    {
        ...
        include(_PS_MODULE_DIR_ . 'statsmodule/statsmodule.php');
        include(_PS_MODULE_DIR_ . 'statsmodule/stats/statssales.php');
        include(_PS_MODULE_DIR_ . 'statsmodule/stats/statsordersprofit.php');
        include(_PS_MODULE_DIR_ . 'statsmodule/stats/statsbestproducts.php');

        $StatsSales = new StatsSales();
        $salesTotal = $StatsSales->getTotals();

        $statsProfit = new StatsOrdersProfit();
        $statsProfit->getData();

        $StatsBestProducts = new StatsBestProducts();
        $StatsBestProducts->getData();

        $profitData = $statsProfit->total;
        $bestProducts = $StatsBestProducts->_values;
        ...
        $graph_data = Hook::exec('dashboardData', $params, $idModule, true, true, (int) Tools::getValue('dashboard_use_push'));

        $graphs = [];

        foreach ($graph_data['dashtrends']['data_chart']['dash_trends_chart1']['data'] as $data) {
            $graphs[ $data['id'] ] = [
                'header' => [],
                'value' => []
            ];
            ...
            foreach ($data['values'] as $value) {
                $graphs[ $data['id'] ]['header'][] = date('m/d/y', $value[0]);
                $graphs[ $data['id'] ]['value'][] = $value[1];

                $total += $value[1];
            }

            $graphs[ $data['id'] ]['total'] = $total;
        }

        Media::addJsDef([
            'StatsOrdersProfit' => $statsProfit->_values,
            'graphs' => $graphs
        ]);

        $this->tpl_view_vars = [
            'CurrencySign' => $this->context->currency->sign,
            'visits' => $graphs['visits']['total'],
            'in_cart' => @$graphs['average_cart_value']['total'],
            'total_profit' => @$graphs['net_profits']['total'],
            'total_orders' => (int) $graphs['orders']['total'],
            'total_sales' => $graphs['sales']['total'],
            'conversion' => number_format($graphs['conversion_rate']['total'], 2),
            'products_sold' => $bestProducts,
        ];
