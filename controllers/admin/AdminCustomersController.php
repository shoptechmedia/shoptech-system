...
class AdminCustomersControllerCore extends AdminController
{
    ...
    public function renderView()
    {
        ...
        $dataset_1 = (new Analytics([

            'method' => 'CustomVariables.getCustomVariablesValuesFromNameId',
            'idSubtable' => 1

        ], '@' . $customer->firstname))->response;

        $dataset_2 = (new Analytics([

            'method' => 'CustomVariables.getCustomVariablesValuesFromNameId',
            'idSubtable' => 2

        ], '@' . $customer->lastname))->response;

        if($dataset_1 && $dataset_2){
            $this->tpl_view_vars['analytics'] = $dataset_1->goals->{'idgoal=ecommerceOrder'};
        }
    ...
    public function printNewsIcon($value, $customer)
    {
        return '<a class="list-action-enable '.($value ? 'action-enabled' : 'action-disabled').'" href="index.php?'.htmlspecialchars(
				...
				'.($value ? '<i class="ion-checkmark"></i>' : '<i class="ion-close"></i>').
            '</a>';
    }
