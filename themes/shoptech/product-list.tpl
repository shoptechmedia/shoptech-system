						...
						{if isset($product.available_for_order) && $product.available_for_order && !isset($restricted_country_mode)}
							<span class="availability availability-slidein {if (isset($product.quantity_all_versions) && $product.quantity_all_versions > 0)}available-diff {/if}">
								{if $product.quantity <= 0}
									{if $PS_STOCK_MANAGEMENT && $product.allow_oosp}
										{if $product.available_later != ''}<span class="out-of-stock">{$product.available_later}</span>{else}<span class="availabile_product">{l s='Available'}</span>{/if}
									{else}
										{if (isset($product.quantity_all_versions) && $product.quantity_all_versions > 0)}
											<span class="available-dif">{l s='Product available with different options'}</span>
										{else}
											<span class="out-of-stock">{l s='Out of stock'}</span>
										{/if}
									{/if}
								{elseif $PS_STOCK_MANAGEMENT}
									<span class="availabile_product">{if isset($product.available_now) && $product.available_now != ''}{$product.available_now}{else}{l s='Available'}{/if}</span>
								{/if}
							</span>
						{/if}
					{/if}
