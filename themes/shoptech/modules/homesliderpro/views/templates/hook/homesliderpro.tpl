		...
		{foreach from=$homeslider_slides item=slide}
			...
			{if $slide.active && $slide.image != ''}
				<li {if $number == 1}class="primo active"{/if} style="padding:0;" data-slide-index="{$number}">
					...
					{/if}
						<picture>
							<source media="(max-width: 767px)" srcset="{$media_server_content_dir}/modules/homesliderpro/images/{$slide.image|escape:'htmlall':'UTF-8'}">

							<img class="SEimage" src="{$media_server_content_dir}/modules/homesliderpro/images/{$slide.image|escape:'htmlall':'UTF-8'}" alt="{$slide.legend|escape:'htmlall':'UTF-8'}" height="{$configuration.height|intval}" width="{$configuration.width|intval}" />
						</picture>
					{if $slide.url != ''}
						</a>
					{/if}
