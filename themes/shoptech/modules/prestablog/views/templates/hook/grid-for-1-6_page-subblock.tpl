...
<section class="block_section clearfix">
    <!-- Module Presta Blog -->
    <h2 class="text-center">{$subblocks.title|escape:'htmlall':'UTF-8'}</h2>
                    ...
                    <div class="block_top">
                        ...
                        {if isset($news_item.image_presente)}
                            {if isset($news_item.link_for_unique)}<a href="{PrestaBlogUrl id=$news_item.id_prestablog_news seo=$news_item.link_rewrite titre=$news_item.title}" class="product_img_link" title="{$news_item.title|escape:'htmlall':'UTF-8'}">{/if}
                                <img src="//static1.prestaspeed.dk{$prestablog_theme_upimg|escape:'html':'UTF-8'}thumb_{$news_item.id_prestablog_news|intval}.jpg" alt="{$news_item.title|escape:'htmlall':'UTF-8'}" />
