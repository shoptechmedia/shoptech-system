...
{if sizeof($news)}
    ...
    <ul id="blog_list" class="list-type">
    {foreach from=$news item=news_item name=NewsName}
        <li>
            <div class="block_cont clearfix">
                <div class="block_left col-xs-12 col-sm-2">
                    <span class="date_blog-cat color_white">
                       {* {l s='Published :' mod='prestablog'}*}

                        {*{dateFormat date=$news_item.date full=0}*}
                        {$post_date = explode('-', $news_item.date)}
                        {$post_date_day = explode(' ', $post_date[2])}
                        <span class="day color_white">{$post_date_day[0]}</span>
                        {DateTime::createFromFormat('!m', $post_date[1])->format("M")}
                        {*{$smarty.now|date_format: $news_item.date}*}

                        
                    </span>
                </div>
                <div class="block_left col-xs-12 col-sm-3 blog-img">
                    ...
                    {if isset($news_item.image_presente)}
                        {if isset($news_item.link_for_unique)}<a href="{PrestaBlogUrl id=$news_item.id_prestablog_news seo=$news_item.link_rewrite titre=$news_item.title}" class="product_img_link" title="{$news_item.title|escape:'htmlall':'UTF-8'}">{/if}
                            <img data-src="//static3.prestaspeed.dk{$prestablog_theme_upimg|escape:'html':'UTF-8'}thumb_{$news_item.id_prestablog_news|intval}.jpg" alt="{$news_item.title|escape:'htmlall':'UTF-8'}" />
                        {if isset($news_item.link_for_unique)}</a>{/if}
                    {/if}
                </div>
                <div class="block_right col-xs-12 col-sm-7 blog_content">
                    <h3>
                        {if isset($news_item.link_for_unique)}<a href="{PrestaBlogUrl id=$news_item.id_prestablog_news seo=$news_item.link_rewrite titre=$news_item.title}" title="{$news_item.title|escape:'htmlall':'UTF-8'}" class="color_black">{/if}{$news_item.title|escape:'htmlall':'UTF-8'}{if isset($news_item.link_for_unique)}</a>{/if}
                        <br />
                    </h3>
                    <span class="blog_misc_wrap">
