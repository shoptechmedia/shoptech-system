...
{if isset($news)}
	...
	{if sizeof($news->categories)}
		...
		{if $prestablog_config.prestablog_uniqnews_rss}<sup><a target="_blank" href="{PrestaBlogUrl rss=$key}"><img src="//static3.prestaspeed.dk{$prestablog_theme_dir|escape:'html':'UTF-8'}/img/rss.png" alt="Rss feed" align="absmiddle" /></a></sup>{/if}
		{if !$smarty.foreach.current.last},{/if}
		{/foreach}
	{/if}
	</p>
	{if isset($news_Image) && $prestablog_config.prestablog_view_news_img}<img src="//static3.prestaspeed.dk{$prestablog_theme_upimg|escape:'html':'UTF-8'}thumb_{$news->id|intval}.jpg" class="news" alt="{$news->title|escape:'htmlall':'UTF-8'}"/>{/if}
	<div id="prestablogfont">{PrestaBlogContent return=str_replace('src', 'data-src', $news->content)}</div>
	<div class="clear"></div>
	...
	<ul style="max-width:490px;" class="rrssb-buttons clearfix">
