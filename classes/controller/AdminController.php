...
class AdminControllerCore extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->bo_css = ((Validate::isLoadedObject($this->context->employee)
            && $this->context->employee->bo_css) ? $this->context->employee->bo_css : 'style.css');
        ...
        $this->context->smarty->assign(
            [
                ...
                'shop_url'         => $this->context->shop->domain_ssl,
            ]
        );
    }
...
//            $pathToImage = false;
    ...
    public function initHeader()
    {
        ...
        if (Validate::isLoadedObject($this->context->employee)) {
            ...
            $this->context->smarty->assign(
                [
                    ...
                    'shop_dashboard'            => $this->context->link->getAdminLink('AdminDashboard'),
                    'newsletter_dashboard'      => $this->context->link->getAdminLink('AdminNewsletter'),
                ]
            );
        } else {
        ...
        foreach ($tabs as $tab) {
            if($tab['current'] && $tab['class_name'] == 'AdminNewsletter'){
                $isDashboard = false;
                $isNewsletter = true;
            }
        }
    ...
    public function getAnalyticsQuery(&$l, $PIWIK_SITEID, $period, $date){
    }
    ...
    public function renderList()
    {
        ...
        foreach ($this->_list as &$l) {
            $this->getAnalyticsQuery($l, $PIWIK_SITEID, $period, $date);
    ...
    public function initFooter()
    {
        ...
        if ($this->context->language->is_rtl) {
            ...
            $this->addCSS($this->theme_path.'/css/'.$this->context->language->iso_code.'.css', 'all', false);
        }
    ...
    public function addJquery($version = NULL, $folder = NULL, $minifier = true){
        ...
        $this->addJS($this->theme_path.'/js/vendors/jquery-3.2.1.min.js', false);
    }

    public function setCSS(){
        //Bootstrap
        $this->addCSS($this->theme_path.'/css/'.$this->bo_css, 'all');
        $this->addCSS($this->theme_path.'/css/color-skins/color1.css', 'all');
        $this->addCSS($this->theme_path.'/css/skins-modes.css', 'all');
        $this->addCSS($this->theme_path.'/plugins/sidemenu/sidemenu.css', 'all');
        $this->addCSS($this->theme_path.'/plugins/sidemenu-responsive-tabs/css/sidemenu-responsive-tabs.css', 'all');
        $this->addCSS($this->theme_path.'/plugins/charts-c3/c3-chart.css', 'all');
        $this->addCSS($this->theme_path.'/plugins/tabs/tabs-2.css', 'all');
        $this->addCSS($this->theme_path.'/plugins/p-scroll/p-scroll.css', 'all');
        $this->addCSS($this->theme_path.'/css/icons.css', 'all');
        $this->addCSS($this->theme_path.'/plugins/right-sidebar/right-sidebar.css', 'all');

        $this->addCSS($this->theme_path.'/css/overrides.css', 'all', PHP_INT_MAX);
    }

    public function setJS(){
        $this->addJquery();
        $this->addJqueryUI(['ui.slider', 'ui.datepicker']);
        $this->addJs(__PS_BASE_URI__.'js/jquery/plugins/timepicker/jquery-ui-timepicker-addon.js');

        //loads specific javascripts for the admin theme
        $this->addJS($this->theme_path.'/js/vendors/bootstrap.bundle.min.js');
        $this->addJS($this->theme_path.'/js/vendors/jquery.sparkline.min.js');
        $this->addJS($this->theme_path.'/js/vendors/circle-progress.min.js');
        $this->addJS($this->theme_path.'/plugins/rating/rating-stars.js');
        $this->addJS($this->theme_path.'/js/popover.js');

        $this->addJS($this->theme_path.'/plugins/chart/chart.bundle.js');
        $this->addJS($this->theme_path.'/plugins/chart/utils.js');

        $this->addJS($this->theme_path.'/plugins/input-mask/input-mask.min.js');
        $this->addJS($this->theme_path.'/plugins/peitychart/jquery.peity.min.js');
        $this->addJS($this->theme_path.'/plugins/peitychart/peitychart.init.js');
        $this->addJS($this->theme_path.'/plugins/peitychart/peitychart.js');
        $this->addJS($this->theme_path.'/plugins/sidemenu/sidemenu.js');
        $this->addJS($this->theme_path.'/plugins/sidemenu-responsive-tabs/js/sidemenu-responsive-tabs.js');
        $this->addJS($this->theme_path.'/js/left-menu.js');

        $this->addJS($this->theme_path.'/plugins/input-mask/input-mask.min.js');
        $this->addJS($this->theme_path.'/plugins/p-scroll/p-scroll.js');
        $this->addJS($this->theme_path.'/plugins/p-scroll/p-scroll-leftmenu.js');
        $this->addJS($this->theme_path.'/plugins/counters/counterup.min.js');
        $this->addJS($this->theme_path.'/plugins/counters/waypoints.min.js');
        $this->addJS($this->theme_path.'/plugins/counters/counters-1.js');
        $this->addJS($this->theme_path.'/plugins/right-sidebar/right-sidebar.js');
    }
    ...
    public function setMedia()
    {
        ...
        if (!$this->lite_display) {
            // $this->addJS($this->theme_path.'/js/help.js');
        }

        if (!Tools::getValue('submitFormAjax')) {
            $this->addJS('https://cdnjs.cloudflare.com/ajax/libs/moment.js/2.24.0/moment-with-locales.min.js');
        ...
        $this->addJS($this->theme_path.'/js/custom.js');
    }
    ...
    public function init()
    {
        ...
        global $currentIndex;
        $this->theme_path = __PS_BASE_URI__.$this->admin_webpath.'/themes/'.$this->bo_theme;
        ...
        if (isset($this->context->cookie->last_activity)) {
            $shortExpire = defined('_TB_COOKIE_SHORT_EXPIRE_') ? _TB_COOKIE_SHORT_EXPIRE_ : 1;
        ...
        if ($this->controller_name != 'AdminLogin' && (!isset($this->context->employee) || !$this->context->employee->id)) {
