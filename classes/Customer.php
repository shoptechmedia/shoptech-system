...
class CustomerCore extends ObjectModel
{
     ...
     * @version 1.0.0 Initial version
     */
    public static function getCustomers($onlyActive = null, $is_newsletter = null, $configurationOrderby = null, $configurationOrderway = null, $filters = [])
    {
        $sql = new DbQuery();
        ...
        if ($is_newsletter) {
            $sql->where('`newsletter` = '.$is_newsletter);
        }

        if (isset($filters['newsletterFilter_id_customer']) && !empty($filters['newsletterFilter_id_customer'])) {
            $sql->where('`id_customer` LIKE "%'.$filters['newsletterFilter_id_customer'].'%"');
        }

        if (isset($filters['newsletterFilter_firstname']) && !empty($filters['newsletterFilter_firstname'])) {
            $sql->where('`firstname` LIKE "%'.$filters['newsletterFilter_firstname'].'%"');
        }

        if (isset($filters['newsletterFilter_lastname']) && !empty($filters['newsletterFilter_lastname'])) {
            $sql->where('`lastname` LIKE "%'.$filters['newsletterFilter_lastname'].'%"');
        }

        if (isset($filters['newsletterFilter_email']) && !empty($filters['newsletterFilter_email'])) {
            $sql->where('`email` LIKE "%'.$filters['newsletterFilter_email'].'%"');
        }

        if ($configurationOrderby) {
            $sql->orderBy('`'.$configurationOrderby.'` '.$configurationOrderway);
        } else {
