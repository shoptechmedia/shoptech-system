...
class CombinationCore extends ObjectModel
{
    ...
    public function getAttributes($idLang)
    {
        return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
            (new DbQuery())
                ->select('a.`id_attribute`, agl.public_name as name, al.name as value')
                ->from('product_attribute_combination', 'pac')
                ->innerJoin('attribute', 'a', 'pac.`id_attribute` = a.`id_attribute`')
                ->innerJoin('attribute_lang', 'al', 'al.`id_attribute` = a.`id_attribute` AND al.id_lang=' . $idLang)
                ->innerJoin('attribute_group', 'ag', 'ag.`id_attribute_group` = a.`id_attribute_group`')
                ->innerJoin('attribute_group_lang', 'agl', 'agl.`id_attribute_group` = ag.`id_attribute_group` AND agl.id_lang=' . $idLang)
                ->where('pac.`id_product_attribute` = '.(int) $this->id)
                // ->where('ag.`is_color_group` = 1')
        );
    }
