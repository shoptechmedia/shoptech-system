	...
	{/if}
		...
		{include file="./front_content_inner.tpl" node=$node }
				...
				{if isset($node.children) && $node.children|@count > 0}
					...
					{foreach from=$node.children item=child name=categoryTreeBranch}
						...
						{if $smarty.foreach.categoryTreeBranch.first}  <div class="tab-content">{/if} 
							{include file="./front_content.tpl" node=$child icount=$i }
			...
			{elseif $node.type==3}
				...
				{if isset($node.children) && $node.children|@count > 0}
				<div id="iqitcontent-tab-id-{$node.elementId}"  class="tab-pane  iqitcontent-element-id-{$node.elementId} clearfix {if $icount == 0} active {/if}">
					{foreach from=$node.children item=child name=categoryTreeBranch}
						{include file="./front_content.tpl" node=$child }
					{/foreach}
				</div>
