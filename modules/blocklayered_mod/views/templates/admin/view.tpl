...
{/if}
<fieldset>
	<legend><img src="{$uri}/img/cogs.gif" alt="{l s='Indexes and caches' mod='blocklayered_mod'}" /> {l s='Indexes and caches' mod='blocklayered_mod'}</legend>
	...
	<div class="row">
		<a type="button" class="ajaxcall-recurcive button" href="{$price_indexer_url}">{l s='Index all missing prices' mod='blocklayered_mod'}</a>
		<a type="button" class="ajaxcall-recurcive button" href="{$full_price_indexer_url}">{l s='Rebuild entire price index' mod='blocklayered_mod'}</a>
		<a type="button" class="ajaxcall button" id="attribute-indexer" rel="attribute" href="{$attribute_indexer_url}">{l s='Build attribute index' mod='blocklayered_mod'}</a>
		<a type="button" class="ajaxcall button" id="url-indexer" rel="price" href="{$url_indexer_url}">{l s='Build URL index' mod='blocklayered_mod'}</a>
	</div>
	...
	<div class="row">
		<div class="info">{l s='A nightly rebuild is recommended.' mod='blocklayered_mod'}</div>
	</div>
</fieldset>
<div class="clearfix">&nbsp;</div>
<fieldset>
	<legend><img src="{$uri}/img/cogs.gif" alt="{l s='Filters templates' mod='blocklayered_mod'}" /> {l s='Filters templates (%d)' sprintf=[$filters_templates|count] mod='blocklayered_mod'}</legend>
	{if $filters_templates|count > 0}
	<div class="row">
		<table class="table table-filter-templates">
			<thead>
				<tr>
					<th class="center"><span class="title_box">{l s='ID' mod='blocklayered_mod'}</span></th>
					<th><span class="title_box text-left">{l s='Name' mod='blocklayered_mod'}</span></th>
					<th class="center"><span class="title_box">{l s='Categories' mod='blocklayered_mod'}</span></th>
					<th><span class="title_box">{l s='Created on' mod='blocklayered_mod'}</span></th>
					<th><span class="title_box text-right">{l s='Actions' mod='blocklayered_mod'}</span></th>
				</tr>
			</thead>
			<tbody>
				...
				<tr>
					...
					<td class="text-right">
						{if empty($limit_warning)}
						<a href="{$current_url}&amp;edit_filters_template=1&amp;id_layered_filter={(int)$template['id_layered_filter']}" class="btn btn-default"><img src="{$uri}img/edit.gif" alt="{l s='Edit' mod='blocklayered_mod'}" />{l s='Edit' mod='blocklayered_mod'}</a> 
						{/if}
						<a href="{$current_url}&amp;deleteFilterTemplate=1&amp;id_layered_filter={(int)$template['id_layered_filter']}"
						onclick="return confirm('{l s='Do you really want to delete this filter template?' mod='blocklayered_mod'}');"><img src="{$uri}img/cross.png" alt="{l s='Delete' mod='blocklayered_mod'}" />{l s='Delete' mod='blocklayered_mod'}</a>
					</td>
				</tr>
				{/foreach}
			</tbody>
	...
	{else}
		<div class="info">{l s='No filter template found.' mod='blocklayered_mod'}</div>
	{/if}
	{if empty($limit_warning)}
	<div class="row">
		<a href="{$current_url}&add_new_filters_template=1" class="button">{l s='Add new filters template' mod='blocklayered_mod'}</a>
	</div>
	{/if}
</fieldset>
<div class="clearfix">&nbsp;</div>
<fieldset>
	<legend><img src="{$uri}/img/cogs.gif" alt="{l s='Configuration' mod='blocklayered_mod'}" /> {l s='Configuration' mod='blocklayered_mod'}</legend>
	<form action="{$current_url}" method="post">
		<table class="table-configurations">
			<tbody>
				<tr>
					<td class="label">
						{l s='Hide filter values when no product is matching' mod='blocklayered_mod'}
					</td>
					<td>
						...
						<label for="ps_layered_hide_0_values_on" class="radioCheck">
							<i class="icon-check-sign color_success"></i> {l s='Yes' mod='blocklayered_mod'}
						</label>
						...
						<label for="ps_layered_hide_0_values_off" class="radioCheck">
							<i class="icon-ban-circle color_danger"></i> {l s='No' mod='blocklayered_mod'}
						</label>
					</td>
				</tr>
				<tr>
					<td class="label">
						{l s='Show the number of matching products' mod='blocklayered_mod'}
					</td>
					<td>
				...
				<tr>
					<td class="label">
						{l s='Show products from subcategories' mod='blocklayered_mod'}
					</td>
					<td>
				...
				<tr>
					<td class="label">
						{l s='Category filter depth (0 for no limits, 1 by default)' mod='blocklayered_mod'}
					</td>
					<td>
				...
				<tr>
					<td class="label">
						{l s='Use tax to filter price' mod='blocklayered_mod'}
					</td>
					<td>
				...
				<tr>
					<td class="label">
						{l s='Allow indexing robots (Google, Yahoo!, Bing, etc.) to use the Condition filter' mod='blocklayered_mod'}
					</td>
					<td>
				...
				<tr>
					<td class="label">
						{l s='Allow indexing robots (Google, Yahoo!, Bing, etc.) to use the Availability filter' mod='blocklayered_mod'}
					</td>
					<td>
				...
				<tr>
					<td class="label">
						{l s='Allow indexing robots (Google, Yahoo!, Bing, etc.) to use the Manufacturer filter' mod='blocklayered_mod'}
					</td>
					<td>
				...
				<tr>
					<td class="label">
						{l s='Allow indexing robots (Google, Yahoo!, Bing, etc.) to use the Category filter' mod='blocklayered_mod'}
					</td>
					<td>
				...
				<tr>
					<td>&nbsp;</td>
					<td>
						<button type="submit" class="btn btn-default" name="submitLayeredSettings"><i class="icon-save"></i> {l s='Save configuration' mod='blocklayered_mod'}</button>
					</td>
				</tr>
			</tbody>
		</table>
