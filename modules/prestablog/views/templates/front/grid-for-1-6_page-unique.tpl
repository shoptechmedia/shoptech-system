...
<div class="single_page">
	<a name="article"></a>
    <div class="date">
        {foreach from=$news->categories item=categorie key=key name=current}<a href="{PrestaBlogUrl c=$key titre=$categorie.link_rewrite}"><span class="guide">{$categorie.title|escape:'htmlall':'UTF-8'}</span></a>
        ...
        <span class="date">{l s='UDGIVET :' mod='prestablog'} {dateFormat date=$news->date full=0}</span>
	...
	{if isset($news_Image)}<img src="{$prestablog_theme_upimg|escape:'html':'UTF-8'}{$news->id|intval}.jpg?{$md5pic|escape:'htmlall':'UTF-8'}" class="news" alt="{$news->title|escape:'htmlall':'UTF-8'}"/>{/if}
	<div id="prestablogfont">
        {PrestaBlogContent return=$news->content}
        <div class="clearfix"></div>
        {if (sizeof($news->products_liaison))}
        <h3>{l s='Related products' mod='prestablog'}</h3>
        <div id="blog_product_linked1" class="blog_product_linked {if $news->is_product_slide_1 === '1'}is_slide{/if}">
                    ...
                    <div class="infowrap">
                        ...
                        {foreach from=$product.features item=feature}
                            {if $feature.id_feature == 8}
                                {assign "replacers" array(' ')}
                                {assign "replaceValues" array('-')}
                                    <img class="feat_img" src="{$img_dir}platform/{$feature.value|escape:'htmlall':'UTF-8'|replace:$replacers:$replaceValues|lower}.png" alt=""/>
                                    <span class="feature_name">{$feature.value}</span>
                            {/if}
                        {/foreach}
                    </div>
                    <a class="button lnk_view btn" href="{$product.link|escape:'html':'UTF-8'}" title="{l s='View'}">
                        <span>{l s='KØB NU'}</span>
                    </a>
    ...
    <div id="prestablogfont2">
        {PrestaBlogContent return=$news->content2}
        <div class="clearfix"></div>
        {if (sizeof($news->products_liaison2))}
            <div id="blog_product_linked2" class="blog_product_linked {if $news->is_product_slide_2 === '1'}is_slide{/if}">
                {foreach from=$news->products_liaison2 item=product key=key name=current}
                <div class="productslinks">
                        <a href="{$product.link|escape:'htmlall':'UTF-8'}">
                            {*PrestaBlogContent return=$product.thumb*}
                            <img src="{$product.thumb}">
    ...
    <div id="prestablogfont3">
        {PrestaBlogContent return=$news->content3}
        <div class="clearfix"></div>
        {if (sizeof($news->products_liaison3))}
            <div id="blog_product_linked3" class="blog_product_linked {if $news->is_product_slide_3 === '1'}is_slide{/if}">
                {foreach from=$news->products_liaison3 item=product key=key name=current}
                <div class="productslinks">
                        <a href="{$product.link|escape:'htmlall':'UTF-8'}">
                            {*PrestaBlogContent return=$product.thumb*}
                            <img src="{$product.thumb}">
	...
	<h3 class="rssb_heaing">{l s='Share this content' mod='prestablog'}</h3>
...
{/if}
    ...
    <ul id="blog_list" class="related_articles">
                ...
                <li class="article_li">
                    <div class="block_cont">
                        ...
                        <div class="block_bas">
                            ...
                            <br /><span class="date_blog-cat">{l s='Udgivet ' mod='prestablog'}
                            ...
                            <p class="blog_description">
                                <a href="{$article.link|escape:'htmlall':'UTF-8'}" title="{$article.title|escape:'htmlall':'UTF-8'}">
                                    {if $article.paragraph!=''}
                                        {$article.paragraph|escape:'htmlall':'UTF-8'}
                                    {/if}
                                </a>
                            </p>
...
{if isset($get_writer)}
    {foreach from=$get_writer item=author}
    <section class="author-wrap">
        <div class="author-left-wrap">
            <img src="{$author_picture}">
        </div>
        <div class="author-right-wrap">
