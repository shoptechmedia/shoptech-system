...
class MailAlerts extends Module
{
    ...
    public function hookActionValidateOrder($params)
    {
        ...
        $p_s = $order->getProducts();
        $link = $this->context->link;
        foreach ($p_s as $p) {
            $p = new Product($p['product_id']);
            $manufacturer = new Manufacturer($p->id_manufacturer);

            $productLink = $link->getProductLink($p);

            $trustpilotProducts[] = '{
                "productUrl": "' . $productLink . '",
                "imageUrl": "https://' . $_SERVER['HTTP_HOST'] . '/' . $p->getCoverWs() . '/cover.jpg",
                "name": "' . $p->name[$this->context->language->id] . '",
                "sku": "' . $p->reference . '",
                "gtin": "' . $p->ean13 . '",
                "mpn": "7TX1641",
        ...
        $trustpilotProducts = implode(',', $trustpilotProducts);

        // Filling-in vars for email
        $templateVars = [
            '{products}'             => $trustpilotProducts,
            '{productReviewInvitationPreferredSendTime}' => date('Y-m-d').'T'.date('H:m:s'),
    ...
    public function hookActionProductOutOfStock($params)
    {
        if (!$this->customer_qty ||
            ...
            ($params['product']->quantity > 0 && $params['product']->available_for_order > 0) /* ||
