...
class CrossSelling_Mod extends Module
{
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->l('Cross-selling modded');
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submitCross')) {
            ...
            } elseif (!($product_nbr = Tools::getValue('CROSSSELLING_NBR_M')) || empty($product_nbr)) {
    ...
    protected function getOrderProducts(array $products_id)
    {
        ...
        if (count($orders) > 0) {
            ...
            $order_products = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
                SELECT DISTINCT od.product_id, pl.name, pl.description_short
                '.(Combination::isFeatureActive() ? ', product_attribute_shop.minimal_quantity AS product_attribute_minimal_quantity, product_shop.id_category_default, IFNULL(product_attribute_shop.`id_product_attribute`,0) as id_product_attribute' : '').'
                , pl.link_rewrite, p.reference, p.available_for_order, p.unity, p.unit_price_ratio, p.customizable, i.id_image, product_shop.show_price,
                    cl.link_rewrite category, p.ean13, stock.out_of_stock, IFNULL(stock.quantity, 0) as quantity, DATEDIFF(
            ...
            foreach ($order_products as &$order_product) {
                ...
                if (($tax_calc == 0 || $tax_calc == 2)) {
                    ...
                    $order_product['price_tax_exc'] =  $order_product['price'];
        ...
        return Product::getProductsProperties((int)$this->context->language->id, $final_products_list);
    }
    ...
    public function hookshoppingCart($params)
    {
        ...
        $cache_id = 'crossselling_mod|shoppingcart|'.implode('|', $products_id);
