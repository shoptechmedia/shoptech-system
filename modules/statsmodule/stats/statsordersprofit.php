...
class StatsOrdersProfit extends StatsModule
{
    ...
    public $query = null;
    public $columns = null;
    public $default_sort_column = null;
    public $default_sort_direction = null;
    public $empty_message = null;
    public $paging_message = null;
    ...
    public function getData($layers = null)
    {
        ...
        $this->query = 'SELECT o.id_order as number, o.invoice_number as invoice_number, DATE_FORMAT(o.invoice_date, \'%Y-%m-%d\') as invoice_date, DATE_FORMAT(o.date_add, \'%Y-%m-%d\') as date_add, ROUND( o.total_paid / o.conversion_rate , 2 ) as paid,  ROUND((o.total_paid / o.conversion_rate + o.total_discounts_tax_incl / o.conversion_rate), 2 ) as total, ROUND((o.total_paid / o.conversion_rate - total_paid_tax_excl / o.conversion_rate) , 2 ) as TaxTotal, ROUND( o.total_shipping_tax_excl / o.conversion_rate , 2 ) AS shipping, ROUND(SUM(o.total_discounts_tax_incl / o.conversion_rate),2) as totalDiscount, ((
        ...
        foreach ($values as &$value) {
            $this->total['paid'] += $value['paid'];
            $this->total['total'] += $value['total'];
            $this->total['shipping'] += $value['shipping'];
            $this->total['TaxTotal'] += $value['TaxTotal'];
            $this->total['cost'] += $value['cost'];
            $this->total['totalDiscount'] += $value['totalDiscount'];
            $this->total['profit'] += $value['profit'];

            $value['paid'] = $value['paid'];
            $value['total'] = $value['total'];
            $value['shipping'] = $value['shipping'];
            $value['TaxTotal'] = $value['TaxTotal'];
            $value['cost'] = $value['cost'];
            $value['totalDiscount'] = $value['totalDiscount'];
            $value['profit'] = $value['profit'];
        }

        $this->total['paid'] = @$this->total['paid'];
        $this->total['total'] = @$this->total['total'];
        $this->total['shipping'] = @$this->total['shipping'];
        $this->total['TaxTotal'] = @$this->total['TaxTotal'];
        $this->total['cost'] = @$this->total['cost'];
        $this->total['totalDiscount'] = @$this->total['totalDiscount'];
        $this->total['profit'] = @$this->total['profit'];
