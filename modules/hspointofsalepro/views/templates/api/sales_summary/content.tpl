...
<div class='report-summary'>
    <table>
        ...
        <tr class="report-generate">
            <td colspan="3">
                ...
                <i>{$hs_pos_i18n.generated_by_at|escape:'htmlall':'UTF-8'|sprintf:$employee_name:$generate_time|replace:'[breakline]':''}</i>
                <br/>
            </td>
        </tr>
        <tr class="space">
    ...
    <table class="sales-summary-content">
        ...
        <tr>
            <td></td>
            <td>{$hs_pos_i18n.refund|escape:'htmlall':'UTF-8'}</td>
            <td class="price">{convertPrice price=$sales_summary.refund}</td>
        </tr>
        <tr>
            <td></td>
            <td>{$hs_pos_i18n.total_real_paid_after_refund|escape:'htmlall':'UTF-8'}</td>
            ...
            {foreach from=$category_summary item=category name=category}
                <tr>
                    ...
                    {else}
                        <td></td>
                    {/if}
                    <td>{$category.name|escape:'htmlall':'UTF-8'}
                        ({(($category.total_price / $sales_summary.total_products) * 100)|string_format:"%.2f"}%)
                    </td>
