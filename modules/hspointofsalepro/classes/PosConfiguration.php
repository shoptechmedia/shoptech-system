...
class PosConfiguration extends Configuration
{
    ...
    public static function getProductSettings()
    {
        $product_settings = array(
            ...
            'POS_DEFAULT_CATEGORIES' => Configuration::get('PS_HOME_CATEGORY'),
    ...
    {
        return array(
            'POS_RECEIPT_LOGO' => Tools::jsonEncode(array(0 => '')),
            'POS_AUTOMATIC_PRINT_QUANTITY' => 1,
            'POS_RECEIPT_FOOTER_TEXT' => Tools::jsonEncode(array(0 => '')),
            'POS_RECEIPT_HEADER_TEXT' => Tools::jsonEncode(array(0 => '')),
            'POS_RECEIPT_BILL_PRINTER_1' => Tools::jsonEncode(array(0 => '')),
            'POS_RECEIPT_BILL_PRINTER_2' => Tools::jsonEncode(array(0 => '')),
            'POS_PRINT_IN_PDF' => Tools::jsonEncode(array(0 => 0)),
            'POS_DEFAULT_TAX_INCLUDED' => Tools::jsonEncode(array(0 => 0)),
            'POS_RECEIPT_LOGO_MAX_WIDTH' => Tools::jsonEncode(array(0 => PosConstants::DEFAULT_RECEIPT_LOGO_MAX_WIDTH)),
            'POS_RECEIPT_PAGE_SIZE' => Tools::jsonEncode(array(0 => PosConstants::DEFAULT_RECEIPT_PAGE_SIZE)),
            'POS_RECEIPT_PRINTER_DPI' => Tools::jsonEncode(array(0 => PosConstants::DEFAULT_PRINTER_DPI)),
            'POS_RECEIPT_TEMPLATE' => Tools::jsonEncode(array(0 => Tools::jsonEncode(self::getDefaultReceiptTemplate()))),
        );
    }
    ...
    public static function getOrderSettings()
    {
        ...
        return array(
            ...
            'POS_ORDER_FIELDS' => Tools::jsonEncode(array(
                $context->employee->id => PosOrder::getDefaultOrderFields()
            )),
        );
    }
    ...
    public static function getReceiptKeys()
    {
        return array(
            ...
            "POS_PRINT_AUTOMATICALLY",
            "POS_PRINT_AUTOMATICALLY_QUANTITY",
            "POS_RECEIPT_TEMPLATE"
        );
    }
     ...
     * @return string
     */
    public static function getValueByEmployeeId($id_employee, $key_config)
    {
        $employee_settings = Tools::jsonDecode(PosConfiguration::get($key_config), true);
        $config_value = '';
        if (is_array($employee_settings) && is_numeric(key($employee_settings))) {
            if (!empty($employee_settings)) {
                $config_value = isset($employee_settings[$id_employee]) ? $employee_settings[$id_employee] : (isset($employee_settings[0]) ? $employee_settings[0] : '');
            }
        } else {
            $config_value = Configuration::get($key_config);
        }
     ...
     * @return bool
     */
    public static function updateValueByEmployeeId($id_employee, $key_config, $value, $html = false)
    {
        $success = array();
        $employee_settings = Tools::jsonDecode(Configuration::get($key_config), true);
        if (is_array($employee_settings) && is_numeric(key($employee_settings))) {
            $employee_settings[$id_employee] = $value;
            $success[] = PosConfiguration::updateValue($key_config, Tools::jsonEncode($employee_settings), $html);
        } else {
            $success[] = PosConfiguration::updateValue($key_config, $value, $html);
        }
