...
class PosOrder extends Order
{
    ...
    public function getProducts($products = false, $selected_products = false, $selected_qty = false, $fullInfos = true)
    {
        ...
        $order_discounts = $this->getCartRules();
        foreach ($products as $index => $product) {
            ...
            $products[$index]['combination'] = $this->getProductCombination($product);
            $products[$index]['manufacturer_name'] = Manufacturer::getNameById($product['id_manufacturer']);
            $products[$index]['product_price_tax_excl'] = $product['product_price']; // @see OrderDetail::setDetailProductPrice()
            $products[$index]['product_price_tax_incl'] = PosProduct::getPriceStatic($id_product, true, $product['product_attribute_id'], 2, null, false, false, 1, false, null, $this->id_cart);

            $products[$index]['price_without_specific_price'] = $products[$index]['product_price_tax_incl']; //@todo To remove this
            if (in_array($product['product_id'], array_column($order_discounts, 'gift_product')) && in_array($product['product_attribute_id'], array_column($order_discounts, 'gift_product_attribute'))) {
                if ($product['product_quantity'] > 1) {
                    $product_gift = array();
                    $product_gift = $product;
                    $product_gift['gift'] = 1;
                    $product_gift['product_quantity'] = 1;
                    $products[$index]['product_quantity'] = $products[$index]['product_quantity'] - 1;
                    $products[$index]['gift'] = 0;
                    array_push($products, $product_gift);
                } else {
                    $products[$index]['gift'] = 1;
                }
            }
            $products[$index]['total_price_tax_incl'] = $product['unit_price_tax_incl'] * $products[$index]['product_quantity'];
            $products[$index]['total_price_tax_excl'] = $product['unit_price_tax_excl'] * $products[$index]['product_quantity'];
            if (!isset($products[$index]['id_customization']) || $products[$index]['id_customization'] == null) {
                $customization = PosProduct::getCustomizedProduct($this->id_cart, $product['id_product'], $product['product_attribute_id'], $product['id_address_delivery']);
                $products[$index]['id_customization'] = count($customization) > 0 ? array_shift(array_keys($customization)) : 0;
            }
        }
    ...
    protected function getProductCombination(array $order_detail)
    {
        ...
        // it should not work well if the combination name include "-", need to research
     ...
     * )
     */
    public static function searchByReference($reference, $page = 1, $limit = null, $pre_order = false)
    {
        if (empty($reference)) {
            return array();
        }
        ...
        $query->leftjoin(
            'order_state_lang',
            'osl',
            'o.`id_lang`= osl.`id_lang` AND osl.`id_order_state` = o.`current_state`'
        );
        ...
        if ($pre_order) {
            $query->where('o.`current_state` = ' . (int)PosConfiguration::get('POS_DEFAULT_PRE_ORDER_STATE'));
        }
     ...
     *               )
     */
    public static function searchByIdCustomer($id_customer = 0, $page = 1, $limit = null, $pre_order = false)
    {
     ...
     *               )
     */
    public static function searchById($id_order, $page = 1, $limit = null, $pre_order = false)
    {
        if (empty($id_order)) {
            return array();
        }
     ...
     * @return type
     */
    public static function cloneOrder($product_returns, PosOrder $order, $payment_method, $amount_due, $shipping_back)
    {
        $context = Context::getContext();
        $new_order = clone $order;
        $gift_products = $order->getCartRules();
        $order_details = self::reCalculateOrderDetails($product_returns, $order->getOrderDetails(), $gift_products);
        $total_price_excl = self::getTotalProducts($order_details);
        $total_price_incl = self::getTotalProducts($order_details, true);
        $order->cancelOrderPayments($payment_method, $amount_due);
        $returned_order = new PosOrderReturn();
        $returned_order->id_returned_order = $order->id;
        $returned_order->id_new_order = $order->id;
        if ($total_price_excl > 0 && $total_price_incl > 0) {
            ...
            if ($new_order->add(false)) {
                                ...
                                if ($customization->add()) {
                                    foreach ($customization_datas as $customization_data) {
                                        PosCustomizationData::add(
                                            $customization_data['index'],
                                            $customization_data['type'],
                                            $customization_data['value'],
                                            $customization->id
                                        );
                                    }
                                }
                ...
                $returned_order->id_new_order = $new_order->id;
                // add order_history
     ...
     * )
     */
    public static function reCalculateOrderDetails(
        array $products,
        array $order_details,
        array $gift_products = array()
    )
    {
        if (!empty($products) && !empty($order_details)) {
            foreach ($order_details as $index => &$order_detail) {
                ...
                if (in_array($order_detail->product_id, array_column($gift_products, 'gift_product')) && in_array($order_detail->product_attribute_id, array_column($gift_products, 'gift_product_attribute'))) {
                    if ($order_detail->product_quantity > 1) {
                        $order_detail->product_quantity = $order_detail->product_quantity - 1;
                        --$order_detail->product_quantity_in_stock;
                        $order_detail->total_price_tax_incl -= $order_detail->unit_price_tax_incl * $order_detail->product_quantity;
                        $order_detail->total_price_tax_excl -= $order_detail->unit_price_tax_excl * $order_detail->product_quantity;
                        $order_detail->id_order_invoice = 0;
                    } else {
                        unset($order_details[$index]);
                    }
     ...
     * @return bool
     */
    public function cancelOrderPayments($payment_method, $amount_due)
    {
        $success = array();

        $cancel_order_payments = $this->getOrderPaymentCollection();

        if ($cancel_order_payments) {
            $new_order_payment = new OrderPayment();
            $new_order_payment->order_reference = $cancel_order_payments[0]->order_reference;
            $new_order_payment->id_currency = $cancel_order_payments[0]->id_currency;
            $new_order_payment->conversion_rate = $cancel_order_payments[0]->conversion_rate;
            $new_order_payment->payment_method = $payment_method;
            $new_order_payment->amount = $amount_due;
            unset($new_order_payment->id);
            $new_order_payment->add();
        }
    ...
    public function getEcoTaxTaxesBreakdown()
    {
        ...
        foreach ($result as $row) {
            if ($row['ecotax_tax_excl'] > 0) {
                $row['ecotax_tax_incl'] = Tools::ps_round(
                    $row['ecotax_tax_excl'] + ($row['ecotax_tax_excl'] * $row['rate'] / 100),
    ...
    )
    {
        ...
        if (!empty($best_seller_products)) {
            foreach ($best_seller_products as &$product) {
                ...
                $product['trends'] = self::getProductTrends(
                    $product['product_id'],
     ...
     *               )
     */
    protected static function getProductTrends(
        $id_product,
     ...
     * )
     */
    public static function getList($module_name, $date_from, $date_to, $current_page = 1, $items_per_page = 18)
    {
        $total_orders = self::getTotalOrders($module_name, $date_from, $date_to);
...
//        $orders = array_merge(
//            self::getOrdersbyModule($module_name, $date_from, $date_to, $start, $items_per_page),
//            self::getOrdersbyModule($module_name, $date_from, $date_to, true)
//        );

        $orders = self::getOrdersbyModule($module_name, $date_from, $date_to, $start, $items_per_page);
        ...
        if (!empty($orders)) {
            foreach ($orders as &$order) {
                ...
                $order['payments'] = OrderPayment::getByOrderReference($order['reference']);
     ...
     * )
     */
    public static function export($module_name, $date_from, $date_to)
    {
        $orders = self::getOrdersbyModule($module_name, $date_from, $date_to, null, null);
     ...
     *               )
     */
    public static function getDefaultOrderFields()
    {
        return array(
            'id' => array(
                'checked' => 1,
     ...
     * )
     */
    protected static function getOrdersbyModule($module_name, $date_from, $date_to, $start, $limit, $return_order = false)
    {
        $sql = new DbQuery();
        $sql->select('o.`id_order`');
        $sql->select('o.`id_currency`');
        ...
        $sql->select('ca.`name` AS `carrier_name`');
        ...
        $sql->from('orders', 'o');
        $sql->leftJoin('pos_sales_commission', 'psc', 'psc.`order_reference` = o.`reference`');
        $sql->leftJoin('employee', 'e', 'psc.`id_employee` = e.`id_employee`');
        $sql->leftJoin('carrier', 'ca', 'ca.`id_carrier` = o.`id_carrier`');
        ...
        $sql->innerJoin(
            'order_state_lang',
            'otl',
            'otl.`id_order_state` = o.`current_state` AND otl.`id_lang` = o.`id_lang`'
        );
    ...
    protected function setInvoiceDetails($order_invoice)
    {
        ...
        if (Configuration::get('PS_ATCP_SHIPWRAP')) {
            ...
            $wrapping_tax_manager = TaxManagerFactory::getManager(
                $address,
        ...
        $order_invoice->saveCarrierTaxCalculator($tax_calculator->getTaxesAmount(
            $order_invoice->total_shipping_tax_excl,
            $order_invoice->total_shipping_tax_incl,
            PosConstants::POS_PRICE_COMPUTE_PRECISION,
            $this->round_mode
        ));
        $order_invoice->saveWrappingTaxCalculator($wrapping_tax_calculator->getTaxesAmount(
     ...
     * )
     */
    public function getTaxes($valid = true)
    {
        $sql = new DbQuery();
     ...
     *               )
     */
    public function getGivenAndReturnMoney()
    {
        $sql = new DbQuery();

        $sql->select('SUM(pcp.`given_money`) AS `given_money`, SUM(pcp.`change`) AS `change`');
        $sql->from('pos_cart_payment', 'pcp');
        $sql->where('pcp.`id_cart` = ' . $this->id_cart);
        return Db::getInstance()->executeS($sql);
    }
     ...
     * @return bool
     */
    public function hasExchangeProducts(array $exchange_products = array())
    {
        $exchange_products_id = array();
        if (!empty($exchange_products)) {
            $exchange_products_id = array_column($exchange_products, 'product_id');
            $products_id = array_column($this->getProducts(), 'product_id');
        }

        return (!empty($exchange_products_id) && count(array_diff($exchange_products_id, $products_id)) == 0) ? true : false;
    }

    /**
     * @return int id_order
     */
    public static function getSampleIdOrder()
    {
        $sql = new DbQuery();
        $sql->select('max(o.`id_order`)');
        $sql->from('orders', 'o');
    ...
    public function getCartRules()
    {
        $sql = new DbQuery();
        $sql->select('orc.*');
        $sql->select('c.`gift_product`');
        $sql->select('c.`gift_product_attribute`');
        $sql->from('order_cart_rule', 'orc');
        $sql->innerJoin('cart_rule', 'c', 'orc.`id_cart_rule` = c.`id_cart_rule`');
        $sql->where('orc.`id_order` = ' . (int)$this->id);
     ...
     * @return mixed
     */
    public static function getTotalOrders($module_name, $date_from, $date_to, $return_order = false)
    {
        $sql = new DbQuery();
