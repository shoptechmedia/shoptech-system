...
class PosPaymentModule extends PaymentModule
{
            ...
            foreach ($order_detail_list as $key => $order_detail) {
                ...
                if (!$order_creation_failed && isset($order->id)) {
                    ...
                    foreach ($order->product_list as $key => $product) {
                        ...
                        $price_wt = PosProduct::getPriceStatic(
                            (int)$product['id_product'],
                            true,
                            ($product['id_product_attribute'] ? (int)$product['id_product_attribute'] : null),
                            2,
				    ...
				    <td style="padding: 0.6em 0.4em;width: 30%;"><strong>' . $product['name'] . (isset($product['attributes']) ? ' - ' . $product['attributes'] : '') . ' - ' . $this->l('Customized') . (!empty($customization_text) ? ' - ' . $customization_text : '') . '</strong></td>
				    ...
				    <td style="padding: 0.6em 0.4em; width: 20%;">' . Tools::displayPrice($customization_quantity * (Product::getTaxCalculationMethod() == PS_TAX_EXC ? Tools::ps_round($price, 2) : $price_wt), $this->context->currency, false) . '</td></tr>';
                        }
				    ...
				    <td style="padding: 0.6em 0.4em; width: 20%;">' . Tools::displayPrice(((int)$product['cart_quantity'] - $customization_quantity) * (Product::getTaxCalculationMethod() == PS_TAX_EXC ? Tools::ps_round($price, 2) : $price_wt), $this->context->currency, false) . '</td></tr>';
                        }
                    ...
                    //should check it is not pre-order state
                    if ((Configuration::get('PS_STOCK_MANAGEMENT') && $order_detail->getStockState() || $order_detail->product_quantity_in_stock < 0) && (int)$order->current_state !== (int)Configuration::get('POS_DEFAULT_PRE_ORDER_STATE')) {
                        $history = new PosOrderHistory();
                        $history->id_order = (int)$order->id;
                        $outofstock_paid = !Configuration::get('POS_DEFAULT_OUT_OF_STOCK_PAID_STATE') ? Configuration::get('PS_OS_OUTOFSTOCK_PAID') : Configuration::get('POS_DEFAULT_OUT_OF_STOCK_PAID_STATE');
                        $outofstock_unpaid = !Configuration::get('POS_DEFAULT_OUT_OF_STOCK_UNPAID_STATE') ? Configuration::get('PS_OS_OUTOFSTOCK_UNPAID') : Configuration::get('POS_DEFAULT_OUT_OF_STOCK_UNPAID_STATE');
                        $history->changeIdOrderState(
                            $order->valid ? $outofstock_paid : $outofstock_unpaid,
                            $order,
                            true
                        );
    ...
    protected function sendEmailToCustomer(
        ...
        $data_order_info = array(
            ...
            '{carrier}' => $this->getTranslation('no_carrier'),
            //$virtual_product ? $carrier->name : Tools::displayError($this->getTranslation('no_carrier')),
