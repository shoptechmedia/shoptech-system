...
class PosReceipt extends Configuration
{
    ...
    public static function getAddress($id_shop, $break_line = false)
    {
        ...
        $state = self::get('POS_RECEIPT_SHOW_STATE') ? State::getNameById(self::get(
            'PS_SHOP_STATE_ID',
            null,
            null,
            $id_shop
    ...
    {
        return self::get('POS_RECEIPT_SHOW_PHONE') && self::get(
            'PS_SHOP_PHONE',
            null,
            null,
            $id_shop
        ) ? self::get('PS_SHOP_PHONE', null, null, $id_shop) : '';
    }
    ...
    {
        return self::get('POS_RECEIPT_SHOW_FAX') && self::get(
            'PS_SHOP_FAX',
            null,
            null,
            $id_shop
        ) ? self::get('PS_SHOP_FAX', null, null, $id_shop) : '';
    }
    ...
    {
        return self::get('POS_RECEIPT_SHOW_REG_NUMBER') && self::get(
            'PS_SHOP_DETAILS',
            null,
            null,
            $id_shop
        ) ? self::get('PS_SHOP_DETAILS', null, null, $id_shop) : '';
    }
     ...
     * @return string
     */
    public static function getLogoFileName($id_employee = 0)
    {
        $logo_file_name = '';
        if (PosConfiguration::getValueByEmployeeId($id_employee, 'POS_RECEIPT_LOGO') && file_exists(_PS_IMG_DIR_ . PosConfiguration::getValueByEmployeeId($id_employee, 'POS_RECEIPT_LOGO'))) {
            $logo_file_name = PosConfiguration::getValueByEmployeeId($id_employee, 'POS_RECEIPT_LOGO');
    ...
    public static function getProductPricesToShow(array $product, $id_currency, $use_tax = false)
    {
        ...
        if (Configuration::get('POS_RECEIPT_SHOW_UNIT_PRICE')) {
            if ($use_tax) {
                $prices[] = $with_currency ? Tools::displayPrice(Tools::displayPrice(
                    $product['product_price_tax_incl'],
                    (int)$id_currency
                )) : sprintf('%.2f', $product['product_price_tax_incl']);
            } else {
                $prices[] = $with_currency ? Tools::displayPrice(Tools::displayPrice(
                    $product['product_price_tax_excl'],
                    (int)$id_currency
                )) : sprintf('%.2f', $product['product_price_tax_excl']);
            }
        }
        if (Configuration::get('POS_RECEIPT_SHOW_PROD_DISCOUNT')) {
            if (!empty($product['reduction_amount']) && $product['reduction_amount'] > 0) {
                $prices[] = $with_currency ? Tools::displayPrice($product['reduction_amount']) : sprintf(
                    '%.2f',
     ...
     * @return int
     */
    public static function getLogoWidth($id_employee = 0)
    {
        return Tools::ps_round(((int)str_replace('K', '', PosConfiguration::getValueByEmployeeId($id_employee, 'POS_RECEIPT_PAGE_SIZE')) * PosConfiguration::getValueByEmployeeId($id_employee, 'POS_RECEIPT_PRINTER_DPI')) / 25.4 * PosConfiguration::getValueByEmployeeId($id_employee, 'POS_RECEIPT_LOGO_MAX_WIDTH') / 100, 0);
    }
     ...
     * @return int
     */
    public static function getPdfPageSize($id_employee = 0)
    {
        return str_replace('K', '', PosConfiguration::getValueByEmployeeId($id_employee, 'POS_RECEIPT_PAGE_SIZE')) * 72 / 25.4;
    }
}
