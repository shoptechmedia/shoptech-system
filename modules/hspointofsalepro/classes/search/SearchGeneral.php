...
class SearchGeneral extends SearchAbstract
{
    ...
    {
        $scored_words = array();
        $words = explode(' ', Search::sanitize($this->keyword, $this->id_lang, false, $this->iso_code));
        if (!count($words)) {
            return array();
        }
        ...
        foreach ($words as $word) {
            if ($this->isKeywordValid($word)) {
                ...
                $eligible_id_products = !empty($eligible_id_products) ? array_intersect(
                    $eligible_id_products,
                    $eligible_id_products_by_keyword
                ) : $eligible_id_products_by_keyword;
                if ($word[0] != '-') {
                    $scored_words[] = $word;
                }
    ...
    public function getProductProperties(array $id_products, array $scored_words = array())
    {
        ...
        Configuration::get('POS_IMAGES_ON_SEARCH_BAR') ? $product_query = $this->getWithImage($product_query) : null;

        $products = Db::getInstance()->executeS($product_query);

        $this->orderByScore($products, $scored_words);
        return $products;
    }
}
