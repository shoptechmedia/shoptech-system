...
class PosAddress extends Address
{
    ...
    public static function createDummyAddress(PosCustomer $customer, $id_shop = null, $phone = null)
    {
        ...
        if (preg_match_all('!\d+!', $phone, $matches)) {
            $phone = !empty($matches) ? max(current($matches)) : '';
        }
        $address->phone = $phone;
     ...
     * @return boolean
     */
    public static function createAddress(PosCustomer $customer, $id_shop = null, $phone = null)
    {
        if (!Validate::isLoadedObject($customer)) {
            return false;
        }
        ...
        foreach ($address_fields_required as $required_field) {
            ...
            switch (self::$definition['fields'][$required_field]['validate']) {
                ...
                case 'isPostCode':
                    ...
                    break;
                case 'isDniLite':
                    $address->$required_field = PosConstants::DEFAULT_DNI_NUMBER;
                    break;
                default:
