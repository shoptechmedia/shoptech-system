...
class PosCategory extends Category
{
    ...
    public function getAllChildren($id_lang = null)
    {
        ...
        $query->innerjoin(
            'category_lang',
            'cl',
            'cl.`id_category` = c.`id_category` AND cl.`id_lang` = ' . (int)$id_lang
        );
        $query->innerjoin(
            'category_shop',
            'cs',
            'cs.`id_category` = c.`id_category` AND cs.`id_shop` = ' . (int)Context::getContext()->shop->id
        );
    ...
    public static function searchProductsById(array $id_categories = array(), $page = 1)
    {
        ...
        $product_query->select('IFNULL(p.`cache_default_attribute`,0) AS `id_product_attribute`');
        ...
        $product_query->leftJoin(
            ...
            'pas',
            'p.`id_product` = pas.`id_product` AND pas.id_shop=' . (int)$context->shop->id
        );
        $product_query->join(PosShop::addSqlAssociation('product', 'p'));
        $product_query->leftJoin(
            'product_lang',
            'pl',
            'p.`id_product` = pl.`id_product` AND pl.`id_lang` = ' . (int)$context->language->id . ' AND pl.`id_shop` = ' . (int)$context->shop->id
        );
        $product_query->leftJoin('image', 'i', 'i.`id_product` = p.`id_product`AND i.`cover` = 1');
        $product_query->leftJoin(
            'image_shop',
            'image_shop',
            'image_shop.`id_image` = i.`id_image` AND image_shop.id_shop=' . (int)$context->shop->id
        );
        $product_query->leftJoin(
            'image_lang',
            'il',
            'image_shop.`id_image` = il.`id_image` AND il.`id_lang` = ' . (int)$context->language->id
        );
        $product_query->join(Product::sqlStock('p', 0, true));
        ...
        $product_query->limit(
            (int)PosConstants::POS_FILTER_PRODUCT_LIMIT,
            (int)$offset * PosConstants::POS_FILTER_PRODUCT_LIMIT
        );
        ...
        foreach ($products as &$product) {
            ...
            $product['image_url'] = $context->link->getImageLink(
                ...
                $product['id_image'],
                PosImageType::getFormattedName('small')
            );
        ...
        if (empty(self::$cache_total_products[implode('-', $id_categories)])) {
            self::$cache_total_products[implode(
                '-',
                $id_categories
            )] = self::getNumberProductsByCategories($id_categories, $context->shop->id);
        }
    ...
    public static function getNumberProductsByCategories(array $id_categories, $id_shop = 0)
    {
        ...
        $query->from('category_product', 'cp');
        $query->leftJoin('product_shop', 'product_shop', 'product_shop.id_product = cp.id_product');
        $query->join(Product::sqlStock('product_shop', 0, true));
        $product_visibilities = PosProduct::getProductVisibilities();
        $query->where(!empty($product_visibilities) ? 'product_shop.`visibility` IN ("' . implode('","', $product_visibilities) . '")' : null);
        $query->where('cp.`id_category` IN (' . implode(',', $id_categories) . ')');
        $query->where(!Configuration::get('POS_PRODUCT_INACTIVE') ? 'product_shop.`active` = 1' : null);
        if (Configuration::get('PS_STOCK_MANAGEMENT')) {
            $query->where(Configuration::get('POS_PRODUCT_OUT_OF_STOCK') ? null : 'stock.`quantity` > 0');
        }
        if ($id_shop > 0) {
            $query->where('product_shop.`id_shop` = ' . (int)$id_shop);
        }
...
//        $sql->where('cl.`id_category` != ' . (int)Configuration::get('PS_HOME_CATEGORY'));
    ...
    public static function getDefaultCategories($id_lang = null)
    {
        ...
        $default_id_categories = array_unique(Configuration::get('POS_DEFAULT_CATEGORIES') ? explode(
            ',',
...
//            ',',
//            Configuration::get('POS_DEFAULT_CATEGORIES')
//        ) : array(Configuration::get('PS_HOME_CATEGORY')));
