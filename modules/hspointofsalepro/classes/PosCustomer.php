...
class PosCustomer extends Customer
{
     ...
     * @param int $shop_id How many customers to be returned
     ...
     *               ...
     */
    public static function search($keyword, $limit = null, array $id_employees = array(), $indexed = false, $shop_id = 0)
    {
        if (!$indexed) {
            ...
            $eligible_id_customers = array();
            foreach ($words as $word) {

                $eligible_id_customers_by_keyword = self::findIdCustomers($word, $shop_id);
                $eligible_id_customers = !empty($eligible_id_customers) ? array_intersect(
                    $eligible_id_customers,
                    $eligible_id_customers_by_keyword
                ) : $eligible_id_customers_by_keyword;
                if ($word[0] != '-') {
                    $scored_words[] = $word;
                }
            ...
            $unique_eligible_id_customers = array_unique($eligible_id_customers);

            if (empty($unique_eligible_id_customers)) {
                return array();
            }

            return self::getCustomerSearchProperties($unique_eligible_id_customers, $words, $shop_id);
        }
    }
     ...
     * @return array
     */
    public static function getCustomerSearchProperties(array $id_customers, array $keywords, $id_shop)
    {
        $db_query = new DbQuery();
        $db_query->select('c.`id_customer`');
        $db_query->select('NULLIF(c.`firstname`,"' . pSQL(PosConstants::NOT_AVAILABLE) . '") AS `firstname`');
        ...
        $db_query->where(Configuration::get('POS_ALLOW_GUEST_SEARCH') ? null : 'c.`is_guest` = 0');
        $db_query->where('1 ' . Shop::addSqlRestriction(Shop::SHARE_CUSTOMER));
        $db_query->where('c.`id_customer` in (' . implode(',', $id_customers) . ')');
        if (!empty($id_employees)) {
            $db_query->where('c.`id_employee` IN(' . implode(',', $id_employees) . ')');
        }

        $customers = Db::getInstance()->executeS($db_query);

        return self::orderByScores($keywords, $id_shop, $customers, $id_customers);
    }

    public static function orderByScores($scored_words, $id_shop, array $customers, array $id_customers)
    {
        $start_search = Configuration::get('PS_SEARCH_START') ? '%' : '';
        $end_search = Configuration::get('PS_SEARCH_END') ? '' : '%';
        $score_query = new DbQuery();
        $score_query->select('SUM(`weight`) weight');
        $score_query->select('pcsi.`id_customer`');
        $score_query->from('pos_customer_search_word', 'pcsw');
        $score_query->leftJoin('pos_customer_search_index', 'pcsi', 'pcsw.`id_word` = pcsi.`id_word`');
        $score_query->where('pcsw.`id_shop` IN (' . $id_shop . ',' . implode(',', Shop::getSharedShops($id_shop, Shop::SHARE_CUSTOMER)) . ')');
        $score_query->where('pcsi.`id_customer` IN (' . implode(',', $id_customers) . ')');
        $score_where = array();
        foreach ($scored_words as $scored_word) {
            $score_where[] = 'pcsw.`word` LIKE \'' . pSQL($start_search) . pSQL(Tools::substr($scored_word, 0, 30)) . pSQL($end_search) . '\'';
        }
        $score_query->where(implode(' OR ', $score_where));
        $score_query->groupBy('pcsi.`id_customer`');

        $scores = Db::getInstance()->executeS($score_query);
        $results = [];
        foreach ($customers as &$customer) {
            foreach ($scores as $score) {
                if ($customer['id_customer'] == $score['id_customer']) {
                    $customer['weight'] = $score['weight'];
                    unset($score);
                }
            }
            if (!isset($customer['weight'])) {
                unset($customer);
            }
        }
     ...
     * @return mixed
     */
    public static function findIdCustomers($keyword, $shop_id)
    {
        $start_search = Configuration::get('PS_SEARCH_START') ? '%' : '';
        $end_search = Configuration::get('PS_SEARCH_END') ? '' : '%';
        $id_customers = array();
        $refined_keyword = str_replace(array('%', '_'), array('\\%', '\\_'), $keyword);
        $searched_word = Tools::substr(
            $refined_keyword,
            ($refined_keyword[0] == '-' ? 1 : 0),
            30
        );
        $query = new DbQuery();
        $query->select('pcsi.`id_customer`');
        $query->from('pos_customer_search_word', 'pcsw');
        $query->leftJoin('pos_customer_search_index', 'pcsi', 'pcsw.`id_word` = pcsi.`id_word`');
        $query->where('pcsw.`id_shop` IN (' . $shop_id . ',' . implode(',', Shop::getSharedShops($shop_id, Shop::SHARE_CUSTOMER)) . ')');
        $query->where('pcsw.`word` LIKE \'' . pSQL($start_search) . pSQL($searched_word) . pSQL($end_search) . '\'');
        foreach (Db::getInstance()->executeS($query) as $row) {
            if (!empty($row['id_customer'])) {
                $id_customers[] = (int)$row['id_customer'];
            }
        }
        return $id_customers;
    ...
    public static function createDummyCustomer($id_employee = null, $id_shop = null, $id_shop_group = null)
    {
        ...
        if (!Validate::isLoadedObject($customer)) {
            ...
            if ($customer->add()) {
                ...
                $default_customer["-1"] = (int)$customer->id;
                if (self::setDefaultCustomerIds($default_customer, $id_shop)) {
    ...
    {
        $default_id_customers = Tools::jsonDecode(
            Configuration::get('POS_DEFAULT_ID_CUSTOMERS', null, null, $id_shop),
            true
        );
        if (is_null($id_employee) && isset($default_id_customers[-1])) {
            return $default_id_customers[-1];
        } elseif ($id_employee && isset($default_id_customers[$id_employee])) {
            return $default_id_customers[$id_employee];
        } elseif ($id_employee && isset($default_id_customers[-1])) {
            return $default_id_customers[-1];
        } else {
            return 0;
        }
    ...
    public static function setDefaultCustomerIds(array $default_customer_ids = array(), $id_shop = null)
    {
        return Configuration::updateValue(
            'POS_DEFAULT_ID_CUSTOMERS',
            Tools::jsonEncode($default_customer_ids),
            false,
            null,
            $id_shop
        );
     ...
     * @return boolean
     */
    public static function addBarcode($id_customer, $barcode)
    {
        $current_date = Date('Y-m-d h:i:s');
        return Db::getInstance()->execute('
        INSERT INTO `' . _DB_PREFIX_ . 'pos_customer` (`id_customer`, `barcode`, `date_add`, `date_update`)
        VALUES (' . (int)$id_customer . ', \'' . pSQL($barcode) . '\', \'' . pSQL($current_date) . '\', \'' . pSQL($current_date) . '\')');
    }
     ...
     * )
     */
    public static function getCustomerByBarcode($barcode)
    {
        $sql = new DbQuery();
        $sql->select('*');
        $sql->from('pos_customer', 'pc');
        $sql->where('pc.`barcode` ="' . pSQL($barcode) . '"');
     ...
     * )
     */
    public static function getBarcodeByIdCustomer($id_customer)
    {
        $sql = new DbQuery();
        $sql->select('pc.`barcode`');
        $sql->from('pos_customer', 'pc');
        $sql->where('pc.`id_customer` =' . (int)$id_customer);
     ...
     * @return array
     */
    public static function getDueAmountByIdCustomer($id_customer)
    {
        $sql = new DbQuery();
        $sql->select('pc.`total_paid`, pc.`total_paid_real`, pc.`id_order`');
        $sql->from('orders', 'pc');
        $sql->where('pc.`id_customer` =' . (int)$id_customer);
        $sql->where('pc.`current_state` =' . (int)PosConfiguration::get('POS_DEFAULT_PRE_ORDER_STATE'));
