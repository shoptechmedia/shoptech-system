...
class PosProduct extends Product
{
    ...
    protected function calculatePrice()
    {
        ...
        if (Configuration::get('PS_TAX') && $this->id_tax_rules_group && $this->tax_included) {
            ...
            $this->price = Tools::ps_round(
                $tax_calculator->removeTaxes($this->price),
    ...
    public static function getCombinations($id_product, $id_shop, $id_lang = null)
    {
        ...
        $query->innerJoin(
            'attribute_shop',
            'ats',
            '(ats.`id_attribute` = a.`id_attribute` AND ats.`id_shop` = ' . (int)$id_shop . ')'
        );
        $query->innerJoin(
            'attribute_lang',
            'al',
     ...
     * ...
     */
    public static function getCustomizedProduct($id_cart, $id_product, $id_product_attribute, $id_address_delivery, $notes = [])
    {
        ...
        foreach ($product_customized as $id_customization => $customization) {
            $note = PosProductNote::findByProductAttribute($notes, $id_product, $id_product_attribute, $id_customization);
            if (isset($customization['datas'])) {
                $customization_datas[$id_customization]['note'] = $note;
    ...
    public static function getSampleProducts()
    {
        ...
        $query->leftJoin(
            'attribute_lang',
            'al',
            'a.`id_attribute` = al.`id_attribute` AND al.`id_lang` = ' . (int)$context->language->id
        );
        $query->leftJoin('attribute_group', 'ag', 'ag.`id_attribute_group` = a.`id_attribute_group`');
        $query->leftJoin(
            'attribute_group_lang',
            'agl',
            'agl.`id_attribute_group` = ag.`id_attribute_group` AND agl.`id_lang` = ' . (int)$context->language->id
        );
        ...
        foreach ($products as $index => &$product) {
            $specific_price_output = null;
            $unit_price = PosProduct::getPriceStatic(
            ...
            $original_price = PosProduct::getPriceStatic(
                (int)$product['id_product'],
                false,
                (int)$product['id_product_attribute'],
                2,
            ...
            $product['product_unit_price'] = PosProduct::reformattedPrice($unit_price, $context->currency);
            $product['product_original_price'] = PosProduct::reformattedPrice($original_price, $context->currency);
            $product['product_total_price'] = PosProduct::reformattedPrice(
                $unit_price * ($index + 1),
                $context->currency
            );
            $product['quantity'] = $index + 1;
            $product['noteProduct'] = 'Note product';
        }
        return $products;
    }
     ...
     * @return string|float
     */
    public static function reformattedPrice($number, $currency, $id_employee = 0)
    {
        $sign = (is_array($currency)) ? $currency['sign'] : (is_object($currency)) ? $currency->sign : '';
        return (PosConfiguration::getValueByEmployeeId($id_employee, 'POS_RECEIPT_SHOW_CURRENCY') == "true") ? Tools::displayPrice($number, $currency) : str_replace($sign, '', Tools::displayPrice($number, $currency));
    }
     ...
     * @param int $id_product
     * @param int $id_product_attribute
     * @return string id_warehouses
     *
     */
    public static function getProductWarehouseIds($id_product, $id_product_attribute)
    {
        $sql = new DbQuery();
        $sql->select("IFNULL(GROUP_CONCAT(`id_warehouse` SEPARATOR ','),0) as id_warehouse");
        $sql->from('stock');
        $sql->where('`id_product` = ' . (int)$id_product);
        $sql->where('`id_product_attribute` = ' . (int)$id_product_attribute);
        $sql->where('`physical_quantity` > 0');
        $sql->orderBy('`id_stock`');
    ...
    {
        $sql = 'INSERT INTO `' . _DB_PREFIX_ . 'category_product` (`id_category`, `id_product`, `position`) VALUES (' . (int)$id_category . ', ' . (int)$this->id . ', 0)';
