...
class PosSalesCommission extends ObjectModel
{
    ...
    public function save($null_values = false, $auto_date = true)
    {
        ...
        return Validate::isLoadedObject($sales_commission) ? $this->update($null_values) : $this->add(
            $auto_date,
            $null_values
        );
    }
     ...
     * )
     */
    public static function getSalesManByIdCart($id_cart)
    {
        $current_saleman = new self($id_cart);
        $salesman = self::getSalesMan();
        if (!empty($salesman) && Validate::isLoadedObject($current_saleman)) {
            foreach ($salesman as &$saleman) {
                $saleman['selected'] = (int)$saleman['id_employee'] == (int)$current_saleman->id_employee;
            }
        }
        return $salesman;
    }
