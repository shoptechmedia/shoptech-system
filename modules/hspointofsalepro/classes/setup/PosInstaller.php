...
class PosInstaller extends PosSetup
{
    ...
    protected function installTables()
    {
        ...
        if (!$this->checkColumnExist('pos_employee_checkin', 'id_shop')) {
            $install_queries[] = 'ALTER TABLE `' . _DB_PREFIX_ . 'pos_employee_checkin` ADD COLUMN `id_shop` integer(10) AFTER `employee_ip`;';
        }
    ...
    protected function installOthers()
    {
        ...
        $success[] = $this->insertDataPosCustomer();
        $success[] = self::installCarrier($this->module);
    ...
    {
        return Tools::copy(
            _PS_ROOT_DIR_ . '/modules/' . $this->module->name . '/views/img/pre-order.gif',
    ...
    protected function createDummyCustomer()
    {
        ...
        foreach ($shops as $shop) {
            $success[] = PosCustomer::createDummyCustomer(
                $context->employee->id,
                (int)$shop['id_shop'],
    ...
    protected function uninstallTables()
    {
        ...
        $uninstall_queries[] = 'DROP TABLE IF EXISTS `' . _DB_PREFIX_ . 'pos_returned_order`';
        $uninstall_queries[] = 'DROP TABLE IF EXISTS `' . _DB_PREFIX_ . 'pos_customer`';
    ...
    /**
     * @param object HsPointOfSalePro $module
     *
     * @return boolean
     */
    public static function installCarrier($module)
    {
        $success = array();
        ...
        foreach ($languages as $language) {
            $carrier->delay[(int)$language['id_lang']] = $module->i18n['pick_up_in_store'];
        }

        if ($carrier->add()) {
            $success[] = Configuration::updateValue('POS_ID_CARRIER', $carrier->id);
            $success[] = array_sum($success) >= count($success) && self::insertCarrierGroup($carrier);
            $success[] = array_sum($success) >= count($success) && self::addToZones($carrier);
            $success[] = array_sum($success) >= count($success) && self::addPriceRange($carrier);
            $success[] = array_sum($success) >= count($success) && self::addWeightRange($carrier);
        } else {
            $success = 0;
        }
     ...
     * @return boolean
     */
    protected static function insertCarrierGroup($carrier)
    {
        $success = array();
     ...
     * @return boolean
     */
    protected static function addToZones($carrier)
    {
        $success = array();
        $zones = Zone::getZones();
     ...
     * @return boolean
     */
    protected static function addPriceRange($carrier)
    {
        $range_price = new RangePrice();
     ...
     * @return boolean
     */
    protected static function addWeightRange($carrier)
    {
        $range_weight = new RangeWeight();
     ...
     * @return bool
     */
    public static function updateConfigForNewShop($module)
    {
        $self = new static($module);
        return $self->installConfigs() && $self->installOtherConfigs();
    }
     ...
     * @return mixed
     */
    public function checkColumnExist($table_name, $column_name)
    {
        return Db::getInstance()->getValue('SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = \'' . _DB_NAME_ . '\' AND TABLE_NAME = \'' . _DB_PREFIX_ . $table_name . '\' AND COLUMN_NAME = \'' . $column_name . '\'');
    }
}
