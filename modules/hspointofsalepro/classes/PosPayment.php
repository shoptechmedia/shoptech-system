...
class PosPayment extends ObjectModel
{
    ...
    public static function getAll($id_lang = null, $id_shop = null, $active = true)
    {
        ...
        $query->leftJoin(
            'pos_payment_lang',
            'pl',
            'p.`id_pos_payment` = pl.`id_pos_payment` AND pl.`id_lang` = ' . (int)$id_lang
        );
        if (!empty($id_shop)) {
            $query->leftJoin('pos_payment_shop', 'ps', 'p.`id_pos_payment` = ps.`id_pos_payment`');
    ...
    public static function getPaymentTypes()
    {
        ...
        $query->from('pos_payment', 'p');
        $query->leftJoin('pos_payment_lang', 'pl', 'p.`id_pos_payment` = pl.`id_pos_payment`');
        $query->leftJoin('pos_payment_shop', 'payment_shop', 'p.`id_pos_payment` = payment_shop.`id_pos_payment`');
        $query->groupBy('p.`id_pos_payment`');
        $query->orderBy('p.`position` ASC');
    ...
    {
        $id_shop_list = Shop::getShops();
        return Db::getInstance()->delete(
            $this->def['table'] . '_shop',
            ' `' . $this->def['primary'] . '`=' . (int)$this->id . ' AND id_shop IN (' . implode(
                ',',
                array_values(array_column($id_shop_list, 'id_shop'))
            ) . ')'
        );
    }
    ...
    public static function getPaymentById($id_payment)
    {
        ...
        $query->where('p.`active` = 1');
        $query->where('p.`id_pos_payment` = ' . (int)$id_payment);
