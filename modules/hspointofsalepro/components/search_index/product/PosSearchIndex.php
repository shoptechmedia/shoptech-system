...
class PosSearchIndex extends Search
{
    ...
    public function run()
    {
        ...
        $id_products_to_index = array_diff($this->getAvailableIdProductsbyOffset($offset), $this->getIndexedIdProducts());
     ...
     *               )</pre>
     */
    protected function getAvailableIdProductsbyOffset($offset)
    {
        $cache_key = __CLASS__ . __FUNCTION__;
        if (!Cache::isStored($cache_key)) {
            $id_products_to_index = array();
