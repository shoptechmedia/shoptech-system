...
(function() {
  ...
  $('#total_price').each(function(index) {
    var price = $(this).html();
    $(this).closest('div').find('div.viabill-pricetag-optional-styles').remove();
    $(this).closest('div').find('div.ViaBill_pricetag_basket').remove();
    $(this).closest('table').after(viaBillGetPrice(price, 'basket', '0px', '0px'));
  });
  ...
  $('#quickpayviabill').each(function(index) {
    ...
    $(this).next().find('span:last-child').html(viaBillGetPrice(price, 'payment', '0px', '0px'));
  });
  {/if}
})();
</script>
