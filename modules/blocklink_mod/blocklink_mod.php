...
public function hookFooter($params)
{
	if (!$this->isCached('blocklink_mod.tpl', $this->getCacheId()))
	{
		...
		$this->smarty->assign(array(
			...
			'url2' => 'url_'.$this->context->language->id,
...
public function addLink()
{
	...
	{
		if (!Db::getInstance()->execute('UPDATE '._DB_PREFIX_.'blocklink_mod SET `new_window` = '.pSQL((int)Tools::getValue('newWindow')).' WHERE `id_blocklink` = '.(int)$id_link))
			return false;
		...
		foreach ($languages as $language)
			...
			{
				if (!Db::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'blocklink_mod_lang VALUES ('.(int)$id_link.', '.(int)($language['id_lang']).', \''.pSQL($_POST['text_'.$language['id_lang']]).'\', \''.pSQL($_POST['url_'.$language['id_lang']]).'\')'))
					return false;
			}
			else
				if (!Db::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'blocklink_mod_lang VALUES ('.(int)$id_link.', '.$language['id_lang'].', \''.pSQL($_POST['text_'.$id_lang_default]).'\', \''.pSQL($_POST['url_'.$id_lang_default]).'\')'))
					return false;
			}
			else
			{
				...
				if (!empty($_POST['text_'.$language['id_lang']]) && !empty($_POST['url_'.$language['id_lang']]))
					{
						...
						if (!Db::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'blocklink_mod_lang VALUES ('.(int)$id_link.', '.(int)($language['id_lang']).', \''.pSQL($_POST['text_'.$id_lang_default]).'\', \''.pSQL($_POST['url_'.$id_lang_default]).'\')'))
						return false;
					}
	...
	public function getLinkById($id)
	{
		if ((int)$id > 0)
		{
			$sql = 'SELECT b.`id_blocklink`, b.`new_window` FROM `'._DB_PREFIX_.'blocklink_mod` b WHERE b.id_blocklink='.$id;
			...
			$results_lang = Db::getInstance()->executeS('SELECT `id_lang`, `text`, `url` FROM '._DB_PREFIX_.'blocklink_mod_lang WHERE `id_blocklink`='.(int)$link['id_blocklink']);
