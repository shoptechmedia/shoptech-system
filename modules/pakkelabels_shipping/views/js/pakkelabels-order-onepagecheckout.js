...
function getShopList(shipping_agent, zipcode)
{
    ...
    if(usedZipCode == zipcode && usedAgent == shipping_agent){
        if(gotError !== ''){
            $('#selected_shop_error').text(gotError);
            return false;
        }
        ...
        success: function(response)
        {
            ...
            if(response){
                ...
                if(returned.status == false)
                {
                    ...
                    $('#selected_shop_error').text(returned.error);
                } else if(returned.status = true)
                {
                    $('#selected_shop_error').text('');
                    if (returned.frontendoption == 'dropdown'){
                         setTimeout(function()
                        {
                            var aaa = $('.pakkelabels-shoplist-dropdownul');
                            if(aaa.length > 0){
                                aaa.remove();
                            }
                           ...
                           jQuery('.pakkelabels-shoplist').addClass('open');
                            ...
                            setTimeout(function(){
                                $('.pakkelabels-shop-list:first-child').click();
                            }, 1);
                        }, 1) 
...
setTimeout(function() { //jQuery(document).ready(function () {
    ...
    if(iPakkelabels_ID_WINDOW=='Popup') {
        ...
        var sZipcodeHTML = '<div class="pakkelabels-div pakkelabels_shipping_field-wrap" id="pakkelabels-zipcode-wrapper"> <div class="pakkelabels_shipping_field"> <div class="pakkelabels-clearfix" id="pakkelabels_shipping_button"> <div class="pakkelabels_stores"> <div> <input type="text" id="Pakkelabels_zipcode_field" name="pakkelabels_zipcode" class="input" placeholder="'+sPakkelabels_zipcode_field+'"> </div> <div id="dropdown-carrier" class="pakkelabels-shoplist dropdown"><button class="btn button-small btn-primary dropdown-toggle" id="pakkelabels_find_shop_btn" type="button" data-toggle="dropdown">' + sPakkelabels_find_shop_btn_text + '<span class="caret"></span></button></div></div> </div> <div id="hidden_choosen_shop" type="hidden"></div></div></div>';

        var sZipcodeHTML2 = '<div class="pakkelabels-div pakkelabels-clearfix" id="selected_shop_wrapper"> <div class="pakkelabels-clearfix" id="selected_shop_header">'+ sSelected_shop_header +'</div> <div class="pakkelabels-clearfix" id="selected_shop_context"> </div></div>';
    }
        ...
        if(checked_shipping == iPakkelabels_ID_GLS || checked_shipping == iPakkelabels_ID_POSTNORD || checked_shipping == iPakkelabels_ID_DAO || checked_shipping == iPakkelabels_ID_BRING) {    
            ...
            if (defaultZipcode) {
                ...
                if (typeof formatedAddressFieldsValuesList === 'undefined' || formatedAddressFieldsValuesList== null) {
                    ...
                    var postcode = formatedAddressFieldsValuesList[postcode];
                    if (address_data) {
    ...
    var Pakkelabels_zipcode_field = document.getElementById('Pakkelabels_zipcode_field');
    $('#postcode').on('blur', function(){
        var that = this;
        setTimeout(function(){
            if(Pakkelabels_zipcode_field.value != that.value){
                $('#Pakkelabels_zipcode_field').val(that.value);

                // $('#pakkelabels_find_shop_btn').click();
            }
        }, 1);
    });
                ...
                if( checked_shipping == iPakkelabels_ID_GLS || checked_shipping == iPakkelabels_ID_DAO || checked_shipping == iPakkelabels_ID_POSTNORD || checked_shipping == iPakkelabels_ID_BRING) {
                    if (checked_shipping == selectedCarrier && hiddenvalue && jQuery('#pakkelabels-zipcode-wrapper').length) {
                        ...
                        if (tempVar) {
                            ...
                            jQuery('<tr class="pakkelabels-choose-shop '+ trs_class +'"><td colspan="'+(tds)+'" style="width:100%">'+sZipcodeHTML2+sZipcodeHTML+'</td></tr>').insertAfter(tempVar);
                        }
   ...
   // shows map
    ...
    jQuery('.cart_quantity_input').on('keyup focusout input change', function () {
        setTimeout(function(){
            $('#pakkelabels_find_shop_btn').click(); 
        }, 1000);
    });
