...
function checkdroppointselected(eventElement){
	...
	jQuery('#delivery-method .continue').show();
...
$('<div class="row carrier-extra-content"></div>').insertAfter('.delivery-option');
	...
	jQuery('#delivery-method').append('<button type="button" class="btn btn-primary pull-xs-right choose-pickuppoint" style="display:none;">'+sPakkelabel_modal_header_h4+'</button>');
    ...
    if(jQuery('.delivery-option input:checked').val() == iPakkelabels_ID_GLS+"," || jQuery('.delivery-option input:checked').val()== iPakkelabels_ID_POSTNORD+"," || jQuery('.delivery-option input:checked').val() == iPakkelabels_ID_DAO+"," || jQuery('.delivery-option input:checked').val() == iPakkelabels_ID_BRING+",")
    {
        ...
        if (jQuery(extra_content).length < 1) {
    		extra_content = jQuery(dev_option).next().append('<div class="row carrier-extra-content"></div>');
    	}
        jQuery(extra_content).delay(1000).html(sZipcodeHTML);
        jQuery('#delivery-method .continue').hide();
    ...
    jQuery('.delivery-option input[type="radio"]:not([value="' + iPakkelabels_ID_GLS + ',"]):not([value="' + iPakkelabels_ID_DAO + ',"]):not([value="' + iPakkelabels_ID_BRING + ',"]):not([value="' + iPakkelabels_ID_POSTNORD + ',"])').on('click', function ()
    {
        jQuery('#pakkelabels-zipcode-wrapper').remove();
