...
<div id="layered_block_left" class="block">
	<h4 class="title_block">{l s='Product filter' mod='stmlayerednavigation'}</h4>
	<div class="block_content">
		<form action="#" id="layered_form">
			<div>
				{if isset($selected_filters) && $n_filters > 0}
				<div id="enabled_filters">
					<span class="layered_subtitle" style="float: none;">{l s='Enabled filters:' mod='stmlayerednavigation'}</span>
					<ul>
								...
								{if $filter.type == $filter_type && isset($filter.values)}
									{if isset($filter.slider)}
										{if $smarty.foreach.f_values.first}
											<li>
												<a href="#" data-rel="layered_{$filter.type}_slider" title="{l s='Cancel' mod='stmlayerednavigation'}">x</a>
												{if $filter.format == 1}
													{l s='%1$s: %2$s - %3$s'|sprintf:$filter.name:{displayPrice price=$filter.values[0]}:{displayPrice price=$filter.values[1]}|escape:html:'UTF-8' mod='stmlayerednavigation'}
												{else}
													{l s='%1$s: %2$s %4$s - %3$s %4$s'|sprintf:$filter.name:$filter.values[0]:$filter.values[1]:$filter.unit|escape:html:'UTF-8' mod='stmlayerednavigation'}
												{/if}
											</li>
										{/if}
									{else}
										{foreach from=$filter.values key=id_value item=value}
											{if $id_value == $filter_key && !is_numeric($filter_value) && ($filter.type eq 'id_attribute_group' || $filter.type eq 'id_feature') || $id_value == $filter_value && $filter.type neq 'id_attribute_group' && $filter.type neq 'id_feature'}
												<li>
													<a href="#" data-rel="layered_{$filter.type_lite}_{$id_value}" title="{l s='Cancel' mod='stmlayerednavigation'}">x</a>
													{l s='%1$s: %2$s' mod='stmlayerednavigation' sprintf=[$filter.name, $value.name]}
												</li>
											{/if}
										{/foreach}
									{/if}
				...
				{foreach from=$filters item=filter}
					{if isset($filter.values)}
							...
							{else}
								...
								{else}
									<select class="select" {if $filter.filter_show_limit > 1}multiple="multiple" size="{$filter.filter_show_limit}"{/if}>
										<option value="">{l s='No filters' mod='stmlayerednavigation'}</option>
										{foreach from=$filter.values key=id_value item=value}
										{if $value.nbr || !$hide_0_values}
						...
						{else}
							{if $filter.filter_type == 0}
								<label for="{$filter.type}">{l s='Range:' mod='stmlayerednavigation'}</label> <span id="layered_{$filter.type}_range"></span>
							...
							{else}
								{if $filter.filter_type == 1}
								<li class="nomargin">
									{l s='From' mod='stmlayerednavigation'} <input class="layered_{$filter.type}_range layered_input_range_min layered_input_range" id="layered_{$filter.type}_range_min" type="text" value="{$filter.values[0]}"/>
									<span class="layered_{$filter.type}_range_unit">{$filter.unit}</span>
									{l s='to' mod='stmlayerednavigation'} <input class="layered_{$filter.type}_range layered_input_range_max layered_input_range" id="layered_{$filter.type}_range_max" type="text" value="{$filter.values[1]}"/>
									<span class="layered_{$filter.type}_range_unit">{$filter.unit}</span>
								...
								{foreach from=$filter.list_of_values  item=values}
									<li class="nomargin {if $filter.values[1] == $values[1] && $filter.values[0] == $values[0]}layered_list_selected{/if} layered_list" onclick="$('#layered_{$filter.type}_range_min').val({$values[0]});$('#layered_{$filter.type}_range_max').val({$values[1]});reloadContent();">
										- {l s='From' mod='stmlayerednavigation'} {$values[0]} {$filter.unit} {l s='to' mod='stmlayerednavigation'} {$values[1]} {$filter.unit}
									</li>
								{/foreach}
						...
						{if count($filter.values) > $filter.filter_show_limit && $filter.filter_show_limit > 0 && $filter.filter_type != 2}
							<span class="hide-action more">{l s='Show more' mod='stmlayerednavigation'}</span>
							<span class="hide-action less">{l s='Show less' mod='stmlayerednavigation'}</span>
						{/if}
						</ul>
					</div>
					<script type="text/javascript">
	...
	<div id="layered_ajax_loader" style="display: none;">
		<p><img src="{$img_ps_dir}loader.gif" alt="" /><br />{l s='Loading...' mod='stmlayerednavigation'}</p>
	</div>
</div>
{else}
<div id="layered_block_left" class="block">
	...
	<div style="display: none;">
		<p style=""><img src="{$img_ps_dir}loader.gif" alt="" /><br />{l s='Loading...' mod='stmlayerednavigation'}</p>
	</div>
</div>
{/if}
