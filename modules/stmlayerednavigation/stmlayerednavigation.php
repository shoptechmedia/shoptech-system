...
/**
 * Class stmlayerednavigation
 */
class stmlayerednavigation extends Module
{
    private $products;
    ...
    /**
     * stmlayerednavigation constructor.
     */
    public function __construct()
    {
        $this->name = 'stmlayerednavigation';
        ...
        $this->displayName = $this->l('STM SEO Navigation');
        $this->description = $this->l('STM SEO Navigation');
    ...
    public function install()
    {
        if (parent::install() && $this->registerHook('header') && $this->registerHook('leftColumn')
            ...
            && $this->registerHook('postProcessFeature') && $this->registerHook('featureValueForm') && $this->registerHook('postProcessFeatureValue') && $this->registerHook('displayFooter')
            && $this->registerHook('afterDeleteFeatureValue') && $this->registerHook('afterSaveFeatureValue') && $this->registerHook('attributeForm') && $this->registerHook('moduleRoutes')
            ...
            Configuration::updateValue('PS_LAYERED_FRIENDLY_URL', 1);
            ...
            $dir = getcwd();
            $admin_dir = end(explode('/', $dir));

            Configuration::updateValue('_PS_ADMIN_DIR_', $admin_dir);
    ...
    public function rebuildLayeredStructure()
    {
        ...
        Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'layered_combinations` (
              `id_combination` int(11) UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
              `id_layered_filter` int(11) NOT NULL,
              `name` text NOT NULL,
              `display_name` text NOT NULL,
              `link_rewrite` text NOT NULL,
              `original_rewrite` text NOT NULL,
    ...
    private static function indexPrices($cursor = null, $full = false, $ajax = false, $smart = false)
    {
        ...
        if (($nb_products > 0 && !$full || $cursor < $nb_products && $full) && !$ajax) {
            $token = substr(Tools::encrypt('stmlayerednavigation/index'), 0, 10);
            ...
            if (!Tools::file_get_contents($domain.__PS_BASE_URI__.'modules/stmlayerednavigation/stmlayerednavigation-price-indexer.php?token='.$token.'&cursor='.(int) $cursor.'&full='.(int) $full)) {
    ...
    public function uninstall()
    {
        ...
        Configuration::deleteByName('PS_LAYERED_FRIENDLY_URL');
    ...
    public function hookDisplayFooter(){
        ...
        $selected_filters = $this->getSelectedFilters();
        $combination = $this->getLayeredCombination($selected_filters);

        if($combination){
            $category = new Category(Tools::getValue('id_category_layered'), $this->context->language);

            if($combination['display_name'] && $combination['display_name'] != $combination['name']){
                $category->name .= ' > ' . $combination['display_name'];
            }else{
                $category->name = '';
            }

            if($combination['description'])
                $category->description = $combination['description'];
            else
                $category->description = '';

            if($combination['image'])
                $banner = '/upload/' . $combination['image'];
            else
                $banner = '';

            if($combination['meta_keywords'])
                $thumbnail = '/upload/' . $combination['thumbnail'];
            else
                $thumbnail = '';
    ...
    private function getSelectedFilters()
    {
        ...
        if (strpos($_SERVER['SCRIPT_FILENAME'], 'stmlayerednavigation-ajax.php') === false || Tools::getValue('selected_filters') !== false) {
            $prefix = _DB_PREFIX_;
            $link_rewrite = Tools::getValue('selected_filters');

            $sql = "
                SELECT * FROM {$prefix}layered_combinations
                WHERE link_rewrite = '{$link_rewrite}'
            ";

            $combination = Db::getInstance()->getRow($sql);

            if($combination){
                $_GET['selected_filters'] = $combination['original_rewrite'];
            }
                    ...
                    if ($attribute_name == 'page') {
                        ...
                        if (in_array($attribute_name, ['price', 'weight'])) {
                            ...
                            foreach ($url_parameters as $url_parameter) {
                                $result = Db::getInstance()->getValue('SELECT data FROM `'._DB_PREFIX_.'layered_friendly_url` WHERE `url_key` = \''.('/'.$attribute_name.$this->getAnchor().$url_parameter).'\'');
    ...
    public function getFilterBlock($selected_filters = [])
    {
        ...
        foreach ($filters as $filter) {
            ...
            foreach ($filters as $filter_tmp) {
                ...
                if (method_exists('stmlayerednavigation', $method_name) &&
        ...
        if ($filter_block = $this->getFilterBlock($selected_filters)) {
            ...
            $smarty->assign(
                [
                    ...
                    'stmlayerednavigationSliderName' => $translate,
                    'col_img_dir'            => _PS_COL_IMG_DIR_,
                ]
            );

            return $this->display(__FILE__, 'stmlayerednavigation.tpl');
        } else {
            return false;
        }
    }
    ...
    public function hookHeader($params)
    {
        ...
        $_GET['id_category_layered'] = $category->id;
        ...
        if($combination){
            if($combination['meta_title'])
                $meta_title = $combination['meta_title'];
            else
                $meta_title = '';

            if($combination['meta_description'])
                $meta_description = $combination['meta_description'];
            else
                $meta_description = '';

            if($combination['meta_keywords'])
                $meta_keywords = $combination['meta_keywords'];
            else
                $meta_keywords = '';

            if($combination['display_name'] && $combination['display_name'] != $combination['name']){
                $meta_title = $category_title . ' > ' . $combination['display_name'];
            }
        ...
        $this->context->controller->addJS(($this->_path).'stmlayerednavigation.js');
        $this->context->controller->addJS('/modules/stmlayerednavigation/stmlayerednavigation-extra.js');
        ...
        $this->context->controller->addCSS(($this->_path).'stmlayerednavigation.css', 'all');
        $this->context->controller->addCSS(($this->_path).'stmlayerednavigation-extra.css', 'all');
    ...
    public function hookFooter($params)
    {
        ...
        if (Dispatcher::getInstance()->getController() == 'category') {
            $this->context->controller->addJS($this->_path.'stmlayerednavigation-footer.js');
        }
    }
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('SubmitFilter')) {
            ...
            if (Tools::isSubmit('submitLayeredSettings')) {
                Configuration::updateValue('PS_LAYERED_FRIENDLY_URL', (int) Tools::getValue('ps_layered_friendly_url'));
        ...
        if (method_exists($this->context->controller, 'addJquery')) {
            $this->context->controller->addJS($this->_path.'js/stmlayerednavigation_admin.js');
            $this->context->controller->addjqueryPlugin('sortable');
        }

        $this->context->controller->addCSS($this->_path.'css/stmlayerednavigation_admin.css');

        if(Tools::isSubmit('edit_seo_template')){
            $prefix = _DB_PREFIX_;
            $db_instance = Db::getInstance();
            $id_layered_filter = Tools::getValue('id_layered_filter');

            if(Tools::isSubmit('id_combination')) {
                $form = $this->context->controller;
                $id_combination = Tools::getValue('id_combination');

                $combination = $db_instance->getRow("
                    SELECT * FROM {$prefix}layered_combinations
                    WHERE id_combination = '{$id_combination}'
                ");

                $banner_image = @$combination['image'];
                $thumbnail_image = @$combination['thumbnail'];
                ...
                if(Tools::isSubmit('delete_banner_image')){
                    ...
                    $db_instance->update('layered_combinations', $toUpdate, 'id_combination=' . $id_combination);

                    exit;
                }

                if(Tools::isSubmit('delete_thumbnail_image')){
                    $toUpdate = [
                        'thumbnail' => ''
                    ];
                ...
                if(Tools::isSubmit('submitSeoCategory')){
                    ...
                    if(isset($_FILES['image']) && !empty($_FILES['image']['name'])){
                        ...
                        $dest = 'SEOCategory-' . $id_combination . '.' . $ext;
                    ...
                    if(isset($_FILES['thumbnail']) && !empty($_FILES['thumbnail']['name'])){
                        ...
                        $thumbnail_image = $dest;
                    }

                    $combination['display_name'] = $display_name;
                    $combination['description'] = $description;
                    $combination['link_rewrite'] = $link_rewrite;
                    $combination['image'] = $banner_image;
                    $combination['thumbnail'] = $thumbnail_image;
                    $combination['meta_title'] = $meta_title;
                    $combination['meta_description'] = $meta_description;
                    $combination['meta_keywords'] = $meta_keywords;
                    ...
                    header('Location: index.php?controller=AdminModules&token=' . Tools::getValue('token') . '&configure=stmlayerednavigation&edit_seo_template=' . Tools::getValue('edit_seo_template') . '&id_layered_filter=' . Tools::getValue('id_layered_filter') . '&id_combination=' . $id_combination);
                    exit;
                }
                ...
                if($banner_image){
                    $input_fields[] = [
                        'type'     => 'html',
                        'html_content' => '<img id="banner_image_visual_aid" src="/upload/' . $banner_image . '" alt=""/><a href="javascript:;" class="delete_banner_image"><i class="process-icon-delete"></i></a>'
                    ];
                }
                ...
                if($thumbnail_image){
                    $input_fields[] = [
                        'type'     => 'html',
                        'html_content' => '<img id="thumbnail_image_visual_aid" src="/upload/' . $thumbnail_image . '" alt=""/><a href="javascript:;" class="delete_thumbnail_image"><i class="process-icon-delete"></i></a>'
                    ];
                }
                ...
                foreach ($input_fields as $input_field) {
                    ...
                    $default_values[$name] = $combination[$name];
                }
                ...
                $form->fields_value = $default_values;

                $form->fields_form = [
                    'tinymce' => true,
                    'class' => 'hidden',
                    'legend'  => [
                        'title' => $this->l('SEO Category') . ': ' . $default_values['name'],
                ...
                $filter = '';
                $name_filter = Tools::getValue('name_filter');

                if($name_filter){
                    $name_filters = explode(',', $name_filter);

                    $filter .= " AND display_name LIKE '%$name_filter%' OR (";
                    foreach ($name_filters as $i => $name_filter) {
                        $name_filter = trim($name_filter);

                        if($i > 0)
                            $filter .= " AND ";

                        $filter .= "name LIKE '%{$name_filter}%'";
                    }

                    $filter .= ')';
                }

                $combinations = $db_instance->ExecuteS("
                    SELECT * FROM {$prefix}layered_combinations
                    WHERE id_layered_filter = '{$id_layered_filter}'{$filter}
                    ORDER BY CHAR_LENGTH(name) ASC, name ASC, id_combination ASC
                    LIMIT {$offset}, {$limit}
                ");

                foreach ($combinations as &$combination) {
                    $combination['name'] = explode(' - ', $combination['name']);
                }
                ...
                if(Tools::isSubmit('ajax')){
                    ...
                    echo $this->display(__FILE__, 'views/templates/admin/seo_list.tpl');
                    return;
                }

                return $this->display(__FILE__, 'views/templates/admin/seo_list.tpl');
            }
        }
        ...
        if (Tools::getValue('add_new_filters_template')) {
            $this->context->smarty->assign(
                [
                    'current_url'       => $this->context->link->getAdminLink('AdminModules').'&configure=stmlayerednavigation&tab_module=front_office_features&module_name=stmlayerednavigation',
                    'uri'               => $this->getPathUri(),
                    'id_layered_filter' => 0,
            ...
            if (Tools::getValue('edit_filters_template')) {
                ...
                $this->context->smarty->assign(
                    [
                        ...
                        'price_indexer_url'      => $module_url.'stmlayerednavigation-price-indexer.php'.'?token='.substr(Tools::encrypt('stmlayerednavigation/index'), 0, 10),
                        'full_price_indexer_url' => $module_url.'stmlayerednavigation-price-indexer.php'.'?token='.substr(Tools::encrypt('stmlayerednavigation/index'), 0, 10).'&full=1',
                        'attribute_indexer_url'  => $module_url.'stmlayerednavigation-attribute-indexer.php'.'?token='.substr(Tools::encrypt('stmlayerednavigation/index'), 0, 10),
                        'url_indexer_url'        => $module_url.'stmlayerednavigation-url-indexer.php'.'?token='.substr(Tools::encrypt('stmlayerednavigation/index'), 0, 10).'&truncate=1',
                        ...
                        'use_friendly_url'       => Configuration::get('PS_LAYERED_FRIENDLY_URL'),
    ...
    public function getLayeredCombination($selected_filters, &$link_rewrite = null, &$link_rewrite2 = null, &$id_layered_filter = 0){
        $prefix = _DB_PREFIX_;
        $db = Db::getInstance();

        $id_layered_filter = 0;
        $id_category_layered = Tools::getValue('id_category_layered');

        $sql = "
            SELECT id_layered_filter, filters FROM {$prefix}layered_filter
        ";

        $available_filters = $db->ExecuteS($sql);

        foreach ($available_filters as $available_filter) {
            $filters = Tools::jsonDecode($available_filter['filters']);

            if(in_array($id_category_layered, (array) $filters->categories)) {
                $id_layered_filter = $available_filter['id_layered_filter'];
            }
        }
        ...
        if($id_layered_filter){
            ...
            foreach ($selected_filters as $filter_type => $filter) {
                foreach ($filter as $id_value => $id_filter) {
                    list($id_filter) = explode('_', $id_filter);
                    ...
                    switch ($filter_type) {
                        case 'category':
                            ...
                            $value = $db->getValue("
                                SELECT cl.name as value FROM {$prefix}category as c
                                INNER JOIN {$prefix}category_lang as cl ON (c.id_category = cl.id_category AND cl.id_lang = '{$this->context->language->id}')
                                WHERE c.id_category = '{$id_filter}'
                            ");
                        ...
                        case 'manufacturer':
                            ...
                            $value = $db->getValue("
                                SELECT name as value FROM {$prefix}manufacturer
                                WHERE id_manufacturer = '{$id_filter}'
                            ");
                        ...
                        case 'id_attribute_group':
                            ...
                            $name = $db->getValue("
                                SELECT name FROM {$prefix}attribute_group_lang
                                WHERE id_attribute_group = '{$id_filter}' AND id_lang = '{$this->context->language->id}'
                            ");

                            $value = $db->getValue("
                                SELECT al.name as value FROM {$prefix}attribute_lang as al
                                WHERE al.id_lang = '{$this->context->language->id}' AND al.id_attribute = '{$id_value}'
                            ");
                        ...
                        case 'id_feature':
                            ...
                            $name = $db->getValue("
                                SELECT name FROM {$prefix}feature_lang
                                WHERE id_feature = '{$id_filter}' AND id_lang = '{$this->context->language->id}'
                            ");

                            $value = $db->getValue("
                                SELECT fvl.value FROM {$prefix}feature_value_lang as fvl
                                WHERE fvl.id_lang = '{$this->context->language->id}' AND fvl.id_feature_value = '{$id_value}'
                            ");
                    ...
                    if($name){
                        $filters[] = [
                            'id' => $id_value,
                            'id_filter' => $name . '_' . $id_value . '_' . $value,
                            'name' => $name,
                            'value' => $value
                        ];
                    }
            ...
            // Arrange Filters
            $id_filter = array_column($filters, 'id_filter');

            array_multisort($id_filter, SORT_ASC, $filters);

            // Get Filters Original Rewrite
            $prev_item = [];
            $link_rewrite = [];
            $l = '';

            foreach ($filters as $i => $filter) {
                $prev_item = $filter[$i - 1];

                if($prev_item['name'] != $filter['name']) {
                    $link_rewrite[] = $l;
                    $l = '';

                    $l .= Tools::str2url($filter['name']);
                }

                $l .= '-' . Tools::str2url($filter['value']);
            }
            ...
            $link_rewrite2 = substr($link_rewrite, 1);

            // Retrieve Combination Details
            $sql = "
                SELECT * FROM {$prefix}layered_combinations
                WHERE original_rewrite = '{$link_rewrite}' OR original_rewrite = '{$link_rewrite2}'
            ";

            $combination = $db->getRow($sql);
    ...
    public function AddCombination () {
        ...
        $cookie = new Cookie('psAdmin');
        $this->context->employee = new Employee($cookie->id_employee);
        ...
        $combination = $this->getLayeredCombination($selected_filters, $link_rewrite, $link_rewrite2, $id_layered_filter);
        ...
        if(!$combination){
            $db->insert(
                ...
                [
                    ...
                    'link_rewrite' => $link_rewrite,
                    'original_rewrite' => $link_rewrite2,
                    'meta_keywords' => $keywords,

                ]
            );

            $id_combination = $db->Insert_ID();
        }else{
            $id_combination = $combination['id_combination'];
        }

        $admin_dir = Configuration::get('_PS_ADMIN_DIR_');

        $token = Tools::getAdminTokenLite('AdminModules', $this->context);

        $link = Tools::getShopDomainSsl(true) . '/' . $admin_dir . '/index.php?controller=AdminModules&token=' . $token . '&configure=stmlayerednavigation&edit_seo_template=' . $id_layered_filter . '&id_layered_filter=' . $id_layered_filter . '&id_combination=' . $id_combination;
    ...
    public function ajaxCall()
    {
        ...
        if ($nb_products == 0) {
            $product_list = $this->display(__FILE__, 'stmlayerednavigation-no-products.tpl');
        } else {
        ...
        if($combination){
            $filter_block['current_friendly_url'] = $combination['link_rewrite'];
            ...
            if($combination['display_name'] && $combination['display_name'] != $combination['name']){
                $heading = $category_title . ' > ' . $combination['display_name'];
            }
            ...
            if($combination['meta_keywords'])
                ...
                $thumbnail = '';
        }elseif($cookie->id_employee){
            // $heading .= $this->editButtton();
        }

        if(!Configuration::get('PS_LAYERED_FRIENDLY_URL')){
            $filter_block['current_friendly_url'] = '';
        }elseif($combination){
            if($filter_block['current_friendly_url']){
                $filters_url = '?selected_filters=' . $filter_block['current_friendly_url'];
            }else{
                $filters_url = '';
            }

            $filter_block['current_friendly_url'] = $filters_url;
        }else{
            if($filter_block['current_friendly_url']){
                $filters_url = '#' . $filter_block['current_friendly_url'];
            }else{
                $filters_url = '';
            }
        ...
        $vars = [
            ...
            'combination_banner' => $banner,
            'combination_thumbnail' => $thumbnail,
    ...
    public function makeCombination($items, $first_index, $itemLimit) {
        $first_item = $items[$first_index];

        $combinations = [];
        $combination = [$first_item];
    ...
    public function makeCombinations($items, $itemLimit) {
        if(!$itemLimit)
            return [];
        ...
        foreach ($items as $index => $item) {
            ...
            $combinations[] = $this->makeCombination($items, $index, $itemLimit);
        }

        return $combinations;
    }

    public function editButtton(){
        return ' <div id="editLayeredCombination"><a href="javascript:;" onclick="editLayeredCombination()">' . $this->l('Edit Combination') . '</a></div>';
    }
}
