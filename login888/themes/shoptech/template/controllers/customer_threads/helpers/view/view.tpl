...
<div class="panel">
	<div class="card-header">
		...
		<h3 class="card-title">{l s="Thread"}:</h3> <span class="badge">#{$id_customer_thread|intval}</span>

		{if isset($next_thread) && $next_thread}
			<a class="btn btn-default ml-auto" href="{$next_thread.href|escape:'html':'UTF-8'}">
...
<div class="panel mt-4">
	<form action="{$link->getAdminLink('AdminCustomerThreads')|escape:'html':'UTF-8'}&amp;id_customer_thread={$thread->id|intval}&amp;viewcustomer_thread" method="post" enctype="multipart/form-data" class="form-horizontal">
		<div class="card-header">
			<h3 class="card-title">{l s="Your answer to"} {if isset($customer->firstname)}{$customer->firstname|escape:'html':'UTF-8'} {$customer->lastname|escape:'html':'UTF-8'} {else} {$thread->email|idnToUtf8|escape:'htmlall':'UTF-8'}{/if}</h3>
		</div>
		...
		<div class="row card-body">
			<div class="media">
				...
				<div class="media-body">
					<textarea class="form-control" cols="30" rows="7" name="reply_message">{$PS_CUSTOMER_SERVICE_SIGNATURE|escape:'html':'UTF-8'}</textarea>
				</div>
			</div>
		</div>

		<div class="card-footer clearfix">
			<button class="btn btn-default pull-right" name="submitReply">{l s='Send'}</button>
...
<div class="panel mt-4">
	<div class="card-header">
		...
		<h3 class="card-title">{l s="Orders and messages timeline"}</h3>
	</div>
