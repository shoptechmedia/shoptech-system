...
<div id="product-images" class="panel product-tab">
	...
	<div class="row panel-body">
		<div class="form-group col-lg-6">
			<label class="control-label file_upload_label">
		...
		<div class="form-group col-lg-6">
			...
			<div class="{if $images|count <= 1} hidden{/if}" id="caption_selection">
				<select class="form-control" name="id_caption">
			...
			{foreach from=$languages item=language}
				...
				<div class="translatable-field row lang-{$language.id_lang}">
					<div class="col-lg-8">
				{/if}
						<input type="text" id="legend_{$language.id_lang}" class="form-control {if isset($input_class)}{$input_class}{/if}" name="legend_{$language.id_lang}" value="{if $images|count}{$images[0]->legend[$language.id_lang]|escape:'html':'UTF-8'}{else}{$product->name[$language.id_lang]|escape:'html':'UTF-8'}{/if}"{if !$product->id} disabled="disabled"{/if}/>
				{if $languages|count > 1}
					</div>
					<div class="col-lg-2">
			...
			<div>
				<button type="submit" class="btn btn-primary" name="submitAddproductAndStay" value="update_legends"><i class="icon-random"></i> {l s='Update'}</button>
			</div>
		</div>
	</div>
	<table class="table table-vcenter tableDnD" id="imageTable">
		<thead>
	...
	<table id="lineType" style="display:none;">
		<tr id="image_id">
			...
			<td class="cover">
				<a href="#">
					<i class="fa fa-square-o icon-2x covered"></i>
				</a>
			</td>
			<td>
				<a href="#" class="delete_product_image pull-right btn btn-danger" >
	...
	<script type="text/javascript">
		...
		function imageLine(id, path, position, cover, shops, legend)
		{
			...
			line = line.replace(/fa fa-square-o/g, cover);
		...
		$(document).ready(function(){
			...
			{foreach from=$images item=image}
				...
				if (assoc != {literal}"{"{/literal})
				{
					...
					assoc = false;
				imageLine({$image->id}, "{$image->getExistingImgPath()}", {$image->position} + '</span>', "{if $image->cover}fa fa-check-square{else}fa fa-square-o{/if}", assoc, "{$image->legend[$default_language]|escape:'htmlall'}");
			{/foreach}
			{literal}
			...
			function afterDeleteProductImage(data)
			{
				...
				if (data)
				{
					...
					if (data.status == 'ok')
					{
						if ($("#" + id + ' .covered').hasClass('fa fa-check-square'))
							cover = 1;
						$("#" + id).remove();
					}
					if (cover)
						$("#imageTable tr").eq(1).find(".covered").addClass('fa fa-check-square');
			...
			$('.image_shop').off().on('click', function(e)
			{
				e.preventDefault();
				active = false;
