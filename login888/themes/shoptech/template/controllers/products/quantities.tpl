...
{if isset($product->id)}
	<div id="product-quantities" class="panel product-tab">
		...
		<h3 class="card-header">{l s='Available quantities for sale'}</h3>
						...
						<div class="col-lg-9 col-lg-offset-3">
							<p class="checkbox">
								...
								{if $stock_management_active == 0 && !$product->cache_is_pack}
									<p class="alert alert-warning"><i class="icon-warning-sign"></i>&nbsp;{l s='This requires you to enable advanced stock management.'}</p>
								{elseif $product->cache_is_pack}
									<p class="alert alert-warning">{l s='When enabling advanced stock management for a pack, please make sure it is also enabled for its product(s) – if you choose to decrement product quantities.'}</p>
								{/if}
						</div>
					</div>
							...
							<table class="table">
								...
								{foreach from=$attributes item=attribute}
									<tr{if isset($attribute['default_on']) && $attribute['default_on']} class="highlighted"{/if}>
										<td class="available_quantity input-group" id="qty_{$attribute['id_product_attribute']}">
											<span class="btn btn-info">{$available_quantity[$attribute['id_product_attribute']]}</span>
											<input class="form-control" type="text" name="qty_{$attribute['id_product_attribute']}" class="fixed-width-sm" value="{$available_quantity[$attribute['id_product_attribute']]|htmlentities}"/>
										</td>
		...
		<div class="panel-footer">
			...
			<button type="submit" name="submitAddproduct" class="btn btn-primary pull-right" disabled="disabled"><i class="process-icon-loading"></i></i> {l s='Save'}</button>
	...
	<br/><div class="panel">
		<h3 class="card-header">{l s='Availability settings'}</h3>
		...
		<div class="panel-body">
			{if !$has_attribute}
				<div class="form-group">
					...
					<div class="col-lg-9">
						...
						<p class="alert alert-warning">{l s='The minimum quantity to buy this product (set to 1 to disable this feature)'}</p>
					</div>
				</div>
			{/if}
			...
			{if $ps_stock_management}
				<div class="form-group">
					...
					<label class="control-label" for="available_now_{$default_language}">
				...
				<div class="form-group">
					...
					<label class="control-label" for="available_later_{$default_language}">
				...
				<div class="form-group">
					...
					<div class="col-lg-9">
						...
						<p class="alert alert-warning">{l s='The next date of availability for this product when it is out of stock.'}</p>
					</div>
				</div>
				{else}
				<div class="form-group">
					<p class="alert alert-warning col-lg-12">{l s='This product has combinations, set eventual availability dates there.'}</p>
				</div>
				{/if}
			{/if}
		</div>
