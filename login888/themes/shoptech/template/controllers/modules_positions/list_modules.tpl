		...
		<div class="">
			...
			<div id="modulePosition">
				<form method="post" action="{$url_submit|escape:'html':'UTF-8'}" >
					...
					<section class="mt-4 card {if $hook['position'] == 0}hook_position{/if}" {if $hook['position'] == 0}style="display:none;"{/if}>
						<a name="{$hook['name']}"></a>
						<header class="card-header">
							<span class="card-title">{$hook['name']}</span><br/>
						...
						<div class="panel-heading">
							<span class="card-subtitle">{$hook['description']}</span>
						</div>
						{/if}
						...
						<ul class="list-unstyled{if $hook['modules']|count > 1} sortable{/if}">
							...
							<li id="{$hook['id_hook']|intval}_{$module['instance']->id|intval}" class="card-body row module_position_{$module['instance']->id|intval} module_list_item{if $can_move && $hook['module_count'] >= 2} draggable{/if}">
								<div class="px-1">
								...
								{if !$display_key}
								<div class="px-1 module_col_position{if $can_move && $hook['module_count'] >= 2} dragHandle{/if}" id="td_{$hook['id_hook']|intval}_{$module['instance']->id}">
