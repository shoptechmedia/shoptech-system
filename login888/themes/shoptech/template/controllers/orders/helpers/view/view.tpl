...
{block name="override_tpl"}
    ...
    <div class="panel-body row">
      <div class="col-xs-6 col-sm-3 box-stats color3">
        <div class="kpi-content">
          <i class="ion-calendar mr-1"></i>
          <span class="title">{l s='Date'}</span>
          <span class="value badge badge-info">{dateFormat date=$order->date_add full=false}</span>
        </div>
      </div>
      <div class="col-xs-6 col-sm-3 box-stats color4">
        <div class="kpi-content">
          <i class="ion-cash mr-1"></i>
          <span class="title">{l s='Total'}</span>
          <span class="value badge badge-info">{displayPrice price=$order->total_paid_tax_incl currency=$currency->id}</span>
        </div>
      </div>
      <div class="col-xs-6 col-sm-3 box-stats color2">
        <div class="kpi-content">
          <i class="ion-chatbubbles mr-1"></i>
          <span class="title">{l s='Messages'}</span>
          <span class="value badge badge-info"><a href="{$link->getAdminLink('AdminCustomerThreads')|escape:'html':'UTF-8'}&amp;id_order={$order->id|intval}">{sizeof($customer_thread_message)}</a></span>
        </div>
      </div>
      <div class="col-xs-6 col-sm-3 box-stats color1">
        <a href="#start_products" class="kpi-content">
          <i class="ion-bag mr-1"></i>
          <span class="title">{l s='Products'}</span>
          <span class="value badge badge-info">{sizeof($products)}</span>
        </a>
      </div>
    </div>
  </div>
    ...
    <div class="col-lg-7">
      <div class="panel">
        <div class="card-header">
          ...
          {l s='Order'}
          <span class="badge badge-info ml-1">{$order->reference}</span>
          <span class="badge badge-info ml-1">{l s="#"}{$order->id}</span>
        ...
        <div class="alert hidden-print">
          ...
          {if Configuration::get('PS_INVOICE') && count($invoices_collection) && $order->invoice_number}
            <a data-selenium-id="view_invoice" class="btn btn-primary _blank" href="{$link->getAdminLink('AdminPdf')|escape:'html':'UTF-8'}&amp;submitAction=generateInvoicePDF&amp;id_order={$order->id|intval}">
              <i class="icon-file"></i>
              {l s='View invoice'}
            </a>
          {else}
          ...
          {if $order->delivery_number}
            <a class="btn btn-primary _blank" href="{$link->getAdminLink('AdminPdf')|escape:'html':'UTF-8'}&amp;submitAction=generateDeliverySlipPDF&amp;id_order={$order->id|intval}">
          ...
          {if Configuration::get('PS_ORDER_RETURN')}
            <a id="desc-order-standard_refund" class="btn btn-primary" href="#refundForm">
          ...
          {/if}
          {if $order->hasInvoice()}
            <a id="desc-order-partial_refund" class="btn btn-primary" href="#refundForm">
              <i class="icon-exchange"></i>
              {l s='Partial refund'}
            </a>
          {/if}
        ...
        <div class="card-body">
          <!-- Tab nav -->
          <ul class="nav nav-tabs ml-0" id="tabOrder">
            {$HOOK_TAB_ORDER}
            <li class="active mr-1">
              <a class="btn btn-default" href="#status">
                <i class="icon-time"></i>
                {l s='Status'} <span class="badge badge-info">{$history|@count}</span>
              </a>
            </li>
            <li>
              <a class="btn btn-default" href="#documents">
                <i class="icon-file-text"></i>
                {l s='Documents'} <span class="badge badge-info">{$order->getDocuments()|@count}</span>
              </a>
            </li>
          </ul>
          <!-- Tab content -->
          <div class="tab-content pt-3">
            ...
            <div class="tab-pane active" id="status">
              ...
              <div class="table-responsive">
                <table class="table table-vcenter history-status row-margin-bottom">
                  <tbody>
      ...
      <div class="mt-4 panel">
        <div class="card-body">
          <!-- Tab nav -->
          <ul class="nav nav-tabs ml-0" id="myTab">
            {$HOOK_TAB_SHIP}
            <li class="active mr-1">
              <a class="btn btn-default" href="#shipping">
                <i class="icon-truck "></i>
                {l s='Shipping'} <span class="badge badge-info">{$order->getShipping()|@count}</span>
              </a>
            </li>
            <li>
              <a class="btn btn-default" href="#returns">
                <i class="icon-undo"></i>
                {l s='Merchandise Returns'} <span class="badge badge-info">{$order->getReturn()|@count}</span>
              </a>
            </li>
          </ul>
          <!-- Tab content -->
            ...
            <div class="tab-pane active" id="shipping">
              ...
              {if !$order->isVirtual()}
                <div class="form-horizontal">
                  ...
                  {if $order->recyclable}
                    <span class="btn badge-success"><i class="text-white ion-checkmark"></i> {l s='Recycled packaging'}</span>
                  {else}
                    <span class="btn badge-default"><i class="text-red ion-close"></i> {l s='Recycled packaging'}</span>
                  {/if}

                  {if $order->gift}
                    <span class="btn badge-success"><i class="text-white ion-checkmark"></i> {l s='Gift wrapping'}</span>
                  {else}
                    <span class="btn badge-default"><i class="text-red ion-close"></i> {l s='Gift wrapping'}</span>
                  {/if}
                </div>
              {/if}
            </div>
      ...
      <!-- Payments block -->
      <div id="formAddPaymentPanel" class="mt-4 panel">
        <div class="card-header">
          <i class="mr-1 fa fa-credit-card"></i>
          {l s="Payment"} <span class="ml-1 badge badge-info">{$order->getOrderPayments()|@count}</span>
        </div>
        ...
        {if (!$order->valid && sizeof($currencies) > 1)}
          <form class="form-horizontal well" method="post" action="{$currentIndex|escape:'html':'UTF-8'}&amp;vieworder&amp;id_order={$order->id}&amp;token={$smarty.get.token|escape:'html':'UTF-8'}">
            <div class="row">
              ...
              <div class="col-lg-3">
                <button type="submit" class="btn btn-primary" name="submitChangeCurrency"><i class="icon-refresh"></i> {l s='Change'}</button>
              </div>
            </div>
          </form>
        {/if}
    ...
    <div class="col-lg-5">
      ...
      <div class="panel">
        {if $customer->id}
          <div class="card-header">
            ...
            <span class="badge badge-info ml-1">
							<a href="?tab=AdminCustomers&amp;id_customer={$customer->id}&amp;viewcustomer&amp;token={getAdminToken tab='AdminCustomers'}" class="text-white">
            ...
            <div class="col-md-6">
              ...
              <div class="panel panel-sm">
                ...
                <form id="customer_note" class="form-horizontal" action="ajax.php" method="post" onsubmit="saveCustomerNote({$customer->id});return false;">
                  <div class="form-group">
                    <div class="col-lg-12">
                      <textarea name="note" id="noteContent" class="form-control textarea-autosize" onkeyup="$(this).val().length > 0 ? $('#submitCustomerNote').removeAttr('disabled') : $('#submitCustomerNote').attr('disabled', 'disabled')">{$customer->note}</textarea>
                    </div>
                  </div>
                  <div class="row">
                    <div class="col-lg-12">
        ...
        <div class="border-top card-body">
          <ul class="nav nav-tabs ml-0" id="tabAddresses">
            <li class="active mr-1">
              <a class="btn btn-default" href="#addressShipping">
                <i class="icon-truck"></i>
                {l s='Shipping address'}
              </a>
            </li>
            <li>
              <a class="btn btn-default" href="#addressInvoice">
                <i class="icon-file-text"></i>
                {l s='Invoice address'}
              </a>
            </li>
                ...
                {if $can_edit}
                  <form class="form-horizontal hidden-print" method="post" action="{$link->getAdminLink('AdminOrders')|escape:'html':'UTF-8'}&amp;vieworder&amp;id_order={$order->id|intval}">
                    <div class="input-group">
                      ...
                      <button class="btn btn-primary" type="submit" name="submitAddressShipping"><i class="text-white icon-refresh"></i> {l s='Change'}</button>
                    </div>
                  </form>
                {/if}
                <div class="well">
              ...
              {if $can_edit}
                <form class="form-horizontal hidden-print" method="post" action="{$link->getAdminLink('AdminOrders')|escape:'html':'UTF-8'}&amp;vieworder&amp;id_order={$order->id|intval}">
                  <div class="input-group">
                    ...
                    <button class="btn btn-primary" type="submit" name="submitAddressInvoice"><i class="text-white icon-refresh"></i> {l s='Change'}</button>
                  </div>
                </form>
              {/if}
              <div class="well">
      ...
      <div class="panel mt-4">
        <div class="card-header">
          <i class="icon-envelope mr-1"></i> {l s='Messages'} <span class="ml-1 badge badge-info">{sizeof($customer_thread_message)}</span>
        </div>
        {if (sizeof($messages))}
        ...
        {/if}
        <div id="messages" class="panel-body hidden-print">
          <form action="{$smarty.server.REQUEST_URI|escape:'html':'UTF-8'}&amp;token={$smarty.get.token|escape:'html':'UTF-8'}" method="post" onsubmit="if (getE('visibility').checked == true) return confirm('{l s='Do you want to send this message to the customer?'}');">
            <div id="message" class="form-horizontal">
              ...
              <div class="form-group">
                <label class="control-label">{l s='Display to customer?'}</label><br/>
              ...
              <div class="form-group">
                ...
                <textarea id="txt_msg" class="form-control textarea-autosize" name="message">{Tools::getValue('message')|escape:'html':'UTF-8'}</textarea>
  ...
  <div class="mt-4 row" id="start_products">
    <div class="col-lg-12">
      <form class="container-command-top-spacing" action="{$current_index}&amp;vieworder&amp;token={$smarty.get.token|escape:'html':'UTF-8'}&amp;id_order={$order->id|intval}" method="post" onsubmit="return orderDeleteProduct('{l s='This product cannot be returned.'}', '{l s='Quantity to cancel is greater than quantity available.'}');">
        ...
        <div class="panel">
          <div class="card-header">
            ...
            {l s='Products'} <span class="ml-1 badge badge-info">{$products|@count}</span>
          </div>
          <div id="refundForm">
            <!--
          ...
          {/if}
          <div class="table-responsive">
            <table class="table table-vcenter" id="orderProducts">
              <thead>
              <tr>
                <th></th>
