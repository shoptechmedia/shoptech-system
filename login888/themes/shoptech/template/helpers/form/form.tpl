...
<form id="{if isset($fields.form.form.id_form)}{$fields.form.form.id_form|escape:'html':'UTF-8'}{else}{if $table == null}configuration_form{else}{$table}_form{/if}{if isset($smarty.capture.table_count) && $smarty.capture.table_count}_{$smarty.capture.table_count|intval}{/if}{/if}" class="defaultForm form-horizontal{if isset($name_controller) && $name_controller} {$name_controller}{/if}"{if isset($current) && $current} action="{$current|escape:'html':'UTF-8'}{if isset($token) && $token}&amp;token={$token|escape:'html':'UTF-8'}{/if}"{/if} method="post" enctype="multipart/form-data"{if isset($style)} style="{$style}"{/if} novalidate>
	...
	{foreach $fields as $f => $fieldset}
		...
		<div class="card panel" id="fieldset_{$f}{if isset($smarty.capture.identifier_count) && $smarty.capture.identifier_count}_{$smarty.capture.identifier_count|intval}{/if}{if $smarty.capture.fieldset_name > 1}_{($smarty.capture.fieldset_name - 1)|intval}{/if}">
				...
				{if $key == 'legend'}
					{block name="legend"}
						<div class="card-header">
							...
							{if isset($field.icon)}<i class="mr-1 {$field.icon}"></i>{/if}
							<h3 class="card-title">{$field.title}</h3>
						</div>
					{/block}
						...
						{else}
							...
							{block name="field"}
								...
								{if $input.type == 'text' || $input.type == 'tags'}
									...
									{foreach $languages as $language}
										...
										{/if}
												...
												<span id="{if isset($input.id)}{$input.id}_{$language.id_lang}{else}{$input.name}_{$language.id_lang}{/if}_counter" class="btn input-group-append btn-info">
									...
									{else}
										...
										{/if}
										{if isset($input.maxchar) && $input.maxchar}
										<span id="{if isset($input.id)}{$input.id}{else}{$input.name}{/if}_counter" class="btn input-group-append btn-info"><span class="text-count-down">{$input.maxchar|intval}</span></span>
										{/if}
										{if isset($input.prefix)}
										<span class="btn input-group-append btn-info">
										  {$input.prefix}
								...
								{elseif $input.type == 'textbutton'}
									...
									<div class="row">
										...
										<div class="input-group">
											<span id="{if isset($input.id)}{$input.id}{else}{$input.name}{/if}_counter" class="btn input-group-append btn-info">
												<span class="text-count-down">{$input.maxchar|intval}</span>
											</span>
										{/if}
										<input type="text"
									...
									<div class="form-group">
										<div class="col-lg-9">
											<div class="form-control-static row">
												<div class="col-xs-6">
													<select {if isset($input.size)}size="{$input.size|escape:'html':'utf-8'}"{/if}{if isset($input.onchange)} onchange="{$input.onchange|escape:'html':'utf-8'}"{/if} class="form-control {if isset($input.class)}{$input.class|escape:'html':'utf-8'}{/if}" id="availableSwap" name="{$input.name|escape:'html':'utf-8'}_available[]" multiple="multiple">
												...
												<div class="col-xs-6">
													<select {if isset($input.size)}size="{$input.size|escape:'html':'utf-8'}"{/if}{if isset($input.onchange)} onchange="{$input.onchange|escape:'html':'utf-8'}"{/if} class="form-control {if isset($input.class)}{$input.class|escape:'html':'utf-8'}{/if}" id="selectedSwap" name="{$input.name|escape:'html':'utf-8'}_selected[]" multiple="multiple">
													{foreach $input.options.query AS $option}
														{if is_object($option)}
															{if in_array($option->$input.options.id, $fields_value[$input.name])}
																<option value="{$option->$input.options.id}">{$option->$input.options.name}</option>
								...
								{elseif $input.type == 'select'}
									...
									{else}
										<select name="{$input.name|escape:'html':'utf-8'}"
												class="form-control {if isset($input.class)}{$input.class|escape:'html':'utf-8'}{/if} fixed-width-xl"
								...
								{elseif $input.type == 'radio'}
									<div class="custom-controls-stacked">
										...
										<label class="custom-control custom-radio">
											<input type="radio" class="custom-control-input" name="{$input.name}" value="{$value.value}" {if $fields_value[$input.name] == $value.value}checked{/if}{if isset($input.disabled) && $input.disabled} disabled="disabled"{/if}>
										...
										{if isset($value.p) && $value.p}<p class="help-block"><small>{$value.p}</small></p>{/if}
									</div>
									...
									{if isset($input.lang) AND $input.lang}
										{foreach $languages as $language}
											...
											{/if}
													...
													{/if}
													<textarea{if isset($input.readonly) && $input.readonly} readonly="readonly"{/if} name="{$input.name}_{$language.id_lang}" id="{if isset($input.id)}{$input.id}{else}{$input.name}{/if}_{$language.id_lang}" class="form-control {if isset($input.autoload_rte) && $input.autoload_rte}rte autoload_rte{else}textarea-autosize{/if}{if isset($input.class)} {$input.class}{/if}"{if isset($input.maxlength) && $input.maxlength} maxlength="{$input.maxlength|intval}"{/if}{if isset($input.maxchar) && $input.maxchar} data-maxchar="{$input.maxchar|intval}"{/if}>{$fields_value[$input.name][$language.id_lang]|escape:'html':'UTF-8'}</textarea>
											{if $languages|count > 1}
												<div class="col-lg-2">
													<button type="button" class="btn btn-default dropdown-toggle" tabindex="-1" data-toggle="dropdown">
														{$language.iso_code}
								...
								{elseif $input.type == 'checkbox'}
									...
									<div class="custom-controls-stacked col-lg-12">
										...
										<label class="custom-control custom-checkbox">
											<input type="checkbox" class="custom-control-input {if isset($input.class)}{$input.class}{/if}" name="{$id_checkbox}" {if isset($value.val)} value="{$value.val|escape:'html':'UTF-8'}"{/if}{if isset($fields_value[$id_checkbox]) && $fields_value[$id_checkbox]} checked="checked"{/if} />
												...
												<div class="form-group">
													...
													<div class="col-lg-9">
														<div class="input-group fixed-width-lg">
															...
															<input type="password" id="{$input.name}" name="{$input.name}" class="form-control {if isset($input.class)}{$input.class}{/if}" value="" required="required" autocomplete="off"/>
														</div>
												...
												<div class="form-group">
													...
													<div class="col-lg-4">
														<div class="input-group fixed-width-lg">
															...
															<input type="password" id="{$input.name}2" name="{$input.name}2" class="form-control {if isset($input.class)}{$input.class}{/if}" value="" autocomplete="off"/>
														</div>
													</div>
												</div>
												<div class="form-group">
								...
								<div class="input-group col-sm-4 ml-0 pl-0">
									...
									<select class="form-control mr-1" name="{$key}" class="fixed-width-lg{if isset($input.class)} {$input.class}{/if}">
										<option value="">-</option>
										{if $key == 'months'}
											{*
									...
									<div class="row">
										<div class="input-group col-lg-4">
											<input
												...
												type="text"
												data-hex="true"
												class="form-control {if isset($input.class)}{$input.class}{else}datepicker{/if}"
												name="{$input.name}"
												value="{$fields_value[$input.name]|escape:'html':'UTF-8'}" />
											<span class="btn input-group-append btn-info">
					...
					<div class="panel-footer">
						...
						{foreach from=$fieldset['form']['buttons'] item=btn key=k}
							{if isset($btn.href) && trim($btn.href) != ''}
								<a href="{$btn.href}" {if isset($btn['id'])}id="{$btn['id']}"{/if} class="btn btn-default{if isset($btn['class'])} {$btn['class']}{/if}" {if isset($btn.js) && $btn.js} onclick="{$btn.js}"{/if}>{$btn.title}</a>
							{else}
								<button type="{if isset($btn['type'])}{$btn['type']}{else}button{/if}" {if isset($btn['id'])}id="{$btn['id']}"{/if} class="btn btn-default{if isset($btn['class'])} {$btn['class']}{/if}" name="{if isset($btn['name'])}{$btn['name']}{else}submitOptions{$table}{/if}"{if isset($btn.js) && $btn.js} onclick="{$btn.js}"{/if}>{$btn.title}</button>
							{/if}
						{/foreach}
						{/if}
					</div>
