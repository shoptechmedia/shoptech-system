...
{if !$simple_header}
	...
	<div class="card panel col-lg-12">
		...
		<div class="card-header">
			{if isset($icon)}<i class="{$icon} mr-1"></i> {/if}<h3 class="card-title">{if is_array($title)}{$title|end}{else}{$title}{/if}</h3>
			{if isset($toolbar_btn) && count($toolbar_btn) >0}
				<span class="badge badge-info ml-1">{$list_total}</span>
				<span class="panel-heading-action ml-auto">
				{foreach from=$toolbar_btn item=btn key=k}
					{if $k != 'modules-list' && $k != 'back'}
						<a id="desc-{$table}-{if isset($btn.imgclass)}{$btn.imgclass}{else}{$k}{/if}" class="mr-1 list-toolbar-btn{if isset($btn.target) && $btn.target} _blank{/if}"{if isset($btn.href)} href="{$btn.href|escape:'html':'UTF-8'}"{/if}{if isset($btn.js) && $btn.js} onclick="{$btn.js}"{/if}>
...
{/if}
	...
	<div class="table-responsive-row clearfix{if isset($use_overflow) && $use_overflow} overflow-y{/if}">
		<table{if $table_id} id="table-{$table_id}"{/if} class="table-vcenter table{if $table_dnd} tableDnD{/if} {$table}" >
			<thead>
					...
					<th class="{if isset($params.class)}{$params.class}{/if}{if isset($params.align)} {$params.align}{/if}">
						<span class="title_box{if isset($order_by) && ($key == $order_by)} active{/if}">
							...
							<span class="d-inline-block">
								<a class="{if isset($order_by) && ($key == $order_by) && ($order_way == 'DESC')}text-gray-dark{/if}" href="{$currentIndex|escape:'html':'UTF-8'}&amp;{$list_id}Orderby={$key|urlencode}&amp;{$list_id}Orderway=desc&amp;token={$token|escape:'html':'UTF-8'}{if isset($smarty.get.$identifier)}&amp;{$identifier}={$smarty.get.$identifier|intval}{/if}">
									<i class="ion-arrow-down-b"></i>
								</a>
								<a class="{if isset($order_by) && ($key == $order_by) && ($order_way == 'ASC')}text-gray-dark{/if}" href="{$currentIndex|escape:'html':'UTF-8'}&amp;{$list_id}Orderby={$key|urlencode}&amp;{$list_id}Orderway=asc&amp;token={$token|escape:'html':'UTF-8'}{if isset($smarty.get.$identifier)}&amp;{$identifier}={$smarty.get.$identifier|intval}{/if}">
				...
				<tr class="nodrag nodrop filter {if $row_hover}row_hover{/if}">
					...
					{foreach $fields_display AS $key => $params}
						<th {if isset($params.align)} class="{$params.align}" {/if}>
							...
							{else}
								{if $params.type == 'bool'}
									<select class="filter fixed-width-sm center form-control" name="{$list_id}Filter_{if isset($params.filter_key)}{$params.filter_key}{else}{$key}{/if}">
										<option value="">-</option>
								...
								{elseif $params.type == 'select'}
									{if isset($params.filter_key)}
										<select class="form-control filter{if isset($params.align) && $params.align == 'center'}center{/if}" onchange="$('#submitFilterButton{$list_id}').focus();$('#submitFilterButton{$list_id}').click();" name="{$list_id}Filter_{$params.filter_key}" {if isset($params.width)} style="width:{$params.width}px"{/if}>
									...
									{/if}
								{else}
									<input type="text" class="form-control filter" name="{$list_id}Filter_{if isset($params.filter_key)}{$params.filter_key}{else}{$key}{/if}" value="{$params.value|escape:'html':'UTF-8'}" {if isset($params.width) && $params.width != 'auto'} style="width:{$params.width}px"{/if} />
								{/if}
							{/if}
						</th>
					{/foreach}
