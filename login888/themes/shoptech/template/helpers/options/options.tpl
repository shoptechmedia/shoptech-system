...
<form action="{$current|escape:'html':'UTF-8'}&amp;token={$token|escape:'html':'UTF-8'}" id="{if $table == null}configuration_form{else}{$table}_form{/if}{if isset($smarty.capture.table_count) && $smarty.capture.table_count}_{$smarty.capture.table_count|intval}{/if}" method="post" enctype="multipart/form-data" class="form-horizontal">
	{foreach $option_list AS $category => $categoryData}
		...
		<div class="panel mb-4 {if isset($categoryData['class'])}{$categoryData['class']}{/if}" id="{$table}_fieldset_{$category}">
			{* Options category title *}
			<div class="card-header">
				<i class="mr-1 {if isset($categoryData['icon'])}{$categoryData['icon']}{else}icon-cogs{/if}"></i>
				<h3 class="card-title">{if isset($categoryData['title'])}{$categoryData['title']}{else}{l s='Options'}{/if}</h3>
			</div>
								...
								{block name="input"}
										...
										<div class="col-lg-9">
											<div class="custom-controls-stacked">
												<label class="custom-control custom-radio">
													<input type="radio" class="custom-control-input" name="{$key}" value="1" {if $field['value']} checked="checked"{/if}{if isset($field['js']['on'])} {$field['js']['on']}{/if}{if isset($field['disabled']) && (bool)$field['disabled']} disabled="disabled"{/if}>
												...
												<label class="custom-control custom-radio">
													<input type="radio" class="custom-control-input" name="{$key}" value="0" {if $field['value']} checked="checked"{/if}{if isset($field['js']['on'])} {$field['js']['on']}{/if}{if isset($field['disabled']) && (bool)$field['disabled']} disabled="disabled"{/if}>
													<span class="custom-control-label">
														{l s='No'}
													</span>
												</label>
									...
									{elseif $field['type'] == 'radio'}
										<div class="custom-controls-stacked col-lg-12">
											...
											<label class="custom-control custom-radio">
												<input type="radio" class="custom-control-input" name="{$key}" value="{$k}"{if $k == $field['value']} checked="checked"{/if}{if isset($field['js'][$k])} {$field['js'][$k]}{/if}>
									...
									{elseif $field['type'] == 'checkbox'}
										...
										<div class="custom-controls-stacked col-lg-12">
											...
											<label class="custom-control custom-checkbox">
												<input type="checkbox" class="custom-control-input" name="{$key}" value="{$k}"{if $k == $field['value']} checked="checked"{/if}{if isset($field['js'][$k])} {$field['js'][$k]}{/if}>
									...
									{elseif $field['type'] == 'password'}
										<div class="col-lg-9">{if isset($field['suffix'])}<div class="input-group{if isset($field.class)} {$field.class}{/if}">{/if}
											<input class="form-control" type="{$field['type']}"{if isset($field['id'])} id="{$field['id']}"{/if} size="{if isset($field['size'])}{$field['size']|intval}{else}5{/if}" name="{$key}" value=""{if isset($field['autocomplete']) && !$field['autocomplete']} autocomplete="off"{/if} />
											{if isset($field['suffix'])}
									...
									{elseif $field['type'] == 'textarea'}
										<div class="col-lg-9">
											<textarea class="form-control textarea-autosize" name="{$key|escape:'htmlall':'UTF-8'}" cols="{$field['cols']|intval}" rows="{$field['rows']|intval}">{$field['value']|escape:'html':'UTF-8'}</textarea>
										</div>
									...
									{elseif $field['type'] == 'color'}
										<div class="col-lg-2">
											<div class="input-group">
												<input type="color" size="{$field['size']}" data-hex="true" {if isset($field.class)}class="form-control {$field['class']}" {else}class="form-control color mColorPickerInput"{/if} name="{$field['name']}" class="{if isset($field['class'])}{$field['class']}{/if}" value="{$field['value']|escape:'html':'UTF-8'}" />
											</div>
							            </div>
										...
										{if $field['type'] == 'textLang'}
											<div class="ml-0 col-lg-9">
												...
												{foreach $field['languages'] AS $id_lang => $value}
													...
													<div class="input-group translatable-field lang-{$id_lang}" {if $id_lang != $current_id_lang}style="display:none;"{/if}>
													{/if}
														<input type="text" class="form-control" 
															name="{$key}_{$id_lang}"
												...
												{foreach $field['languages'] AS $id_lang => $value}
													<div class="row translatable-field lang-{$id_lang}" {if $id_lang != $current_id_lang}style="display:none;"{/if}>
														<div id="{$key}_{$id_lang}" class="col-lg-9" >
															<textarea class="form-control textarea-autosize" name="{$key}_{$id_lang}">{$value|replace:'\r\n':"\n"}</textarea>
														</div>
														<div class="col-lg-2">
															<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">
																{foreach $languages as $language}
										...
										{elseif $field['type'] == 'selectLang'}
											{foreach $languages as $language}
												<div id="{$key}_{$language.id_lang}" style="display: {if $language.id_lang == $current_id_lang}block{else}none{/if};" class="col-lg-9">
													<select class="form-control" name="{$key}_{$language.iso_code|upper}">
